...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = MtBotCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = MtBotCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = MtBotCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = MtBotCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = MtBotCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__mtbot__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: MtBotLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for mtbot:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case MtBotLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case MtBotCanvasCommand.present.rawValue,
                 MtBotCanvasCommand.hide.rawValue,
                 MtBotCanvasCommand.navigate.rawValue,
                 MtBotCanvasCommand.evalJS.rawValue,
                 MtBotCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case MtBotCanvasA2UICommand.reset.rawValue,
                 MtBotCanvasA2UICommand.push.rawValue,
                 MtBotCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case MtBotCameraCommand.list.rawValue,
                 MtBotCameraCommand.snap.rawValue,
                 MtBotCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case MtBotScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: MtBotNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: MtBotNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(MtBotLocationGetParams.self, from: req.paramsJSON)) ??
            MtBotLocationGetParams()
        ...
        let payload = MtBotLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case MtBotCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(MtBotCanvasPresentParams.self, from: req.paramsJSON)) ??
                MtBotCanvasPresentParams()
        ...
        case MtBotCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case MtBotCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(MtBotCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case MtBotCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(MtBotCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case MtBotCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(MtBotCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case MtBotCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              ...
              if (!host) return JSON.stringify({ ok: false, error: "missing mtbotA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case MtBotCanvasA2UICommand.push.rawValue, MtBotCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == MtBotCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(MtBotCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try MtBotCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(MtBotCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try MtBotCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.mtbotA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case MtBotCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case MtBotCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(MtBotCameraSnapParams.self, from: req.paramsJSON)) ??
                MtBotCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case MtBotCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(MtBotCameraClipParams.self, from: req.paramsJSON)) ??
                MtBotCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(MtBotScreenRecordParams.self, from: req.paramsJSON)) ??
            MtBotScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> MtBotLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return MtBotLocationMode(rawValue: raw) ?? .off
    }

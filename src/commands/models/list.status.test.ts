...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveMtBotAgentDir: vi.fn().mockReturnValue("/tmp/mtbot-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/mtbot-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/mtbot-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveMtBotAgentDir: mocks.resolveMtBotAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveMtBotAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/mtbot-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/mtbot-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/mtbot-agent-custom");

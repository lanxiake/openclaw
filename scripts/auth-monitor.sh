...
#
# Suggested cron: */30 * * * * /home/admin/mtbot/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send MtBot notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., mtbot-alerts)
...
STATE_FILE="$HOME/.mtbot/auth-monitor-state"
...
send_notification() {
    ...
    # Send via MtBot if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use mtbot
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via MtBot to $NOTIFY_PHONE..."
            mtbot send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! MtBot is down. Run: ssh l36 '~/mtbot/scripts/mobile-reauth.sh'" "urgent"
    exit 1

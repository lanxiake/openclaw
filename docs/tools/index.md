---
summary: "Agent tool surface for MtBot (browser, canvas, nodes, message, cron) replacing legacy `mtbot-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `mtbot-*` skills
---

# Tools (MtBot)

MtBot exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `mtbot-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `mtbot.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, MtBot logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`mtbot node run`).
...
- Requires a Brave API key (recommended: `mtbot configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated MtBot-managed browser.
...
- Quick smoke: `mtbot nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `mtbot gateway` restart in-place)

...
const defaultTools = createMtBotCodingTools();

describe("createMtBotCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mtbot-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createMtBotCodingTools();
    ...
    try {
      ...
      const contents = "Hello from mtbot read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "mtbot-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "mtbot-workspace"),
    ...
    const tools = createMtBotCodingTools({ sandbox });

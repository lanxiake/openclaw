...
async function testWAAuth002(page) {
  ...
  try {
    ...
    const hasError = pageContent.includes('失败') || pageContent.includes('错误') || pageContent.includes('invalid');

    if (stillOnLogin) {
      recordResult(id, name, 'PASS', `仍在登录页, 错误提示${hasError ? '已显示' : '未检测到文字但未跳转'}`);
    } else {
...
async function testWAAuth003(page) {
  ...
  try {
    ...
    await page.goto(`${CONFIG.baseUrl}/devices`, { waitUntil: 'networkidle', timeout: CONFIG.timeout });
...
async function testWAAuth004(page) {
  ...
  try {
    ...
    const userMenuButton = page.locator('button:has-text("用户"), button:has(svg + span + svg)').first();
    ...
    const tokenAfterLogout = await page.evaluate(() => {
      return localStorage.getItem('access_token') || localStorage.getItem('admin_access_token') || null;
    });
...
async function testWAAuth005(page) {
  ...
  try {
    ...
    for (const target of navTargets) {
      try {
        await page.goto(`${CONFIG.baseUrl}${target.url}`, { waitUntil: 'networkidle', timeout: CONFIG.timeout });
    ...
    if (passedNav >= navTargets.length - 1) {
      ...
      recordResult(id, name, 'FAIL', `路由导航仅 ${passedNav}/${navTargets.length} 通过 [${detail}]`);
    }
  } catch (err) {
    await takeScreenshot(page, `${id}-error`);
    recordResult(id, name, 'FAIL', err.message);
...
async function testWADev002(page) {
  ...
  try {
    ...
    const pageContent = await page.textContent('body');
    const hasContent = pageContent.includes('设备') || pageContent.includes('暂无') || pageContent.includes('没有');
...
async function testWADev003(page) {
  ...
  try {
    ...
    if (buttons > 0) {
      recordResult(id, name, 'PASS', `设备页面有 ${buttons} 个可交互按钮, 搜索框: ${searchInput > 0 ? '有' : '无'}`);
    } else {
...
async function testWASkill003(page) {
  ...
  try {
    await page.goto(`${CONFIG.baseUrl}/skills/my`, { waitUntil: 'networkidle', timeout: CONFIG.timeout });
    await page.waitForTimeout(2000);
    await takeScreenshot(page, `${id}-01-my-skills`);

    const currentUrl = page.url();
    const pageContent = await page.textContent('body');
    const hasContent = pageContent.includes('技能') || pageContent.includes('暂无') || pageContent.includes('我的');
...
async function testWASub001(page) {
  ...
  try {
    await page.goto(`${CONFIG.baseUrl}/subscription`, { waitUntil: 'networkidle', timeout: CONFIG.timeout });
    await page.waitForTimeout(2000);
...
async function testWASub002(page) {
  ...
  try {
    ...
    const pageContent = await page.textContent('body');
    const hasCards = await page.locator('[class*="card"], [class*="Card"]').count();
    const hasPlanInfo = pageContent.includes('计划') || pageContent.includes('Plan') ||
                        pageContent.includes('免费') || pageContent.includes('专业') || pageContent.includes('基础');
...
async function testWAGW001(page) {
  ...
  try {
    ...
    const pageContent = await page.textContent('body');
    const hasContent = pageContent.includes('仪表盘') || pageContent.includes('Dashboard') ||
                       pageContent.includes('OpenClaw') || pageContent.includes('管理');
...
async function testWAGW002(page) {
  ...
  try {
    ...
    const pageContent = await page.textContent('body');
    const hasData = pageContent.includes('设备') || pageContent.includes('暂无') || pageContent.includes('没有');
...
async function testWAGW003(page) {
  ...
  try {
    ...
    const pageContent = await page.textContent('body');
    const hasContent = pageContent.includes('仪表盘') || pageContent.includes('Dashboard') || pageContent.includes('OpenClaw');

...
import type { HealthStatus, IMemoryProvider, ProviderConfig } from './interfaces/memory-provider.js'
 ...
 * 记忆管理器状态
 */
export type MemoryManagerStatus = 'uninitialized' | 'initializing' | 'ready' | 'degraded' | 'shutdown'
...
export class MemoryManager {
  ...
  async shutdown(): Promise<void> {
    ...
    if (this._working) {
      shutdownTasks.push(this._working.shutdown().catch(e => {
        console.error('[MemoryManager] working 提供者关闭失败:', e)
      }))
    }
    if (this._episodic) {
      shutdownTasks.push(this._episodic.shutdown().catch(e => {
        console.error('[MemoryManager] episodic 提供者关闭失败:', e)
      }))
    }
    if (this._profile) {
      shutdownTasks.push(this._profile.shutdown().catch(e => {
        console.error('[MemoryManager] profile 提供者关闭失败:', e)
      }))
    }
    if (this._knowledge) {
      shutdownTasks.push(this._knowledge.shutdown().catch(e => {
        console.error('[MemoryManager] knowledge 提供者关闭失败:', e)
      }))
    }
    if (this._storage) {
      shutdownTasks.push(this._storage.shutdown().catch(e => {

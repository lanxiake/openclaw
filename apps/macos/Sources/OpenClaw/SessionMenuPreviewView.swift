...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "com.yunkun.mtbot", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> MtBotSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: MtBotSessionPreviewEntry,
    ...
    private static func cache(payload: MtBotSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: MtBotChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [MtBotKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [MtBotKit.AnyCodable]) -> [MtBotChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(MtBotChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: MtBotChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: MtBotChatMessage) -> Bool {
    ...
    private static func toolNames(for message: MtBotChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: MtBotChatMessage) -> String? {

...
beforeEach(() => {
  ...
  resolveMtBotPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.MTBOT_STATE_DIR;
  originalUpdateInProgress = process.env.MTBOT_UPDATE_IN_PROGRESS;
  process.env.MTBOT_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mtbot-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.MTBOT_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.MTBOT_UPDATE_IN_PROGRESS;
  } else {
    process.env.MTBOT_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveMtBotPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadMtBotPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/mtbot-root.js", () => ({
  resolveMtBotPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "mtbot-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.MTBOT_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when MTBOT_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/mtbot.json",
      exists: true,
      raw: "{}",
    ...
    process.env.MTBOT_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();

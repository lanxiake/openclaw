...
const SANDBOX_EXAMPLES = {
  main: [
    ["mtbot sandbox list", "List all sandbox containers."],
    ["mtbot sandbox list --browser", "List only browser containers."],
    ["mtbot sandbox recreate --all", "Recreate all containers."],
    ["mtbot sandbox recreate --session main", "Recreate a specific session."],
    ["mtbot sandbox recreate --agent mybot", "Recreate agent containers."],
    ["mtbot sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["mtbot sandbox list", "List all sandbox containers."],
    ["mtbot sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["mtbot sandbox recreate --all", "Recreate all containers."],
    ["mtbot sandbox recreate --session main", "Recreate a specific session."],
    ["mtbot sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["mtbot sandbox recreate --browser --all", "Recreate only browser containers."],
    ["mtbot sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["mtbot sandbox explain", "Show effective sandbox config."],
    ["mtbot sandbox explain --session agent:main:main", "Explain a specific session."],
    ["mtbot sandbox explain --agent work", "Explain an agent sandbox."],
    ["mtbot sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.mtbot.top/cli/sandbox")}\n`,
    )
    .action(() => {

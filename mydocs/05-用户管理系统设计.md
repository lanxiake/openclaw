# AI 个人助理平台 - 用户管理系统设计文档

> 版本: 2.0 | 创建日期: 2026-02-07 | 更新日期: 2026-02-07 | 状态: 评审后修订
>
> 本文档补充 03-技术方案文档.md 中用户管理相关的缺失部分

---

## 修订记录

| 版本 | 日期 | 修订内容 |
|------|------|----------|
| 1.0 | 2026-02-07 | 初始设计 |
| 2.0 | 2026-02-07 | 根据评审反馈修订：重构用户-设备关系、简化认证架构、增强安全机制、补充数据迁移策略 |

---

## 1. 概述

### 1.1 背景

现有 OpenClaw 代码采用**设备配对 + 会话隔离**模式，缺少完整的用户账号体系。本文档设计用户管理系统，补充以下能力：

| 设计文档描述 | 现有实现 | 本文档补充 |
|--------------|----------|------------|
| PostgreSQL 存储用户数据 | JSON 文件存储 | ✅ 完整数据库设计 |
| User 实体 (phone, email) | 基于设备配对，无用户实体 | ✅ 用户账号体系 |
| JWT Token | 简单 Token，非标准 JWT | ✅ 标准 JWT 认证 |
| Subscription 订阅系统 | 未实现 | ✅ 订阅与计费系统 |
| AuditLog 审计日志 | 操作日志但非数据库存储 | ✅ 结构化审计系统 |

### 1.2 设计原则

1. **渐进式集成** - 与现有 Gateway 认证、设备配对机制兼容
2. **最小侵入** - 新增模块，不修改现有核心代码
3. **职责分离** - 用户管身份，设备管权限（关键改进）
4. **可扩展性** - 支持未来多租户、企业版扩展
5. **安全优先** - 遵循 OWASP 安全最佳实践

### 1.3 核心设计决策

#### 1.3.1 用户与设备的职责分离

```
┌─────────────────────────────────────────────────────────────────┐
│                      核心设计原则                                │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   用户 (User)              设备 (Device)                        │
│   ├── 身份标识             ├── 操作权限 (role, scopes)          │
│   ├── 登录凭据             ├── 设备 Token                       │
│   ├── 订阅状态             ├── 平台信息                         │
│   └── 个人资料             └── 最后活跃时间                      │
│                                                                 │
│   用户登录 → 获取 User Token (用于身份验证)                      │
│   设备配对 → 获取 Device Token (用于操作授权，复用现有机制)       │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

**关键点**：
- 用户表**不存储** role 和 scopes（这些属于设备）
- 设备表**关联**到现有 device-pairing.ts，而非重复存储
- 一个用户可以绑定多个设备，每个设备可以有不同权限

#### 1.3.2 双 Token 认证架构

```
┌─────────────────────────────────────────────────────────────────┐
│                      认证 Token 架构                             │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   User Access Token (JWT)        Device Token (现有机制)         │
│   ├── 用途: 身份验证             ├── 用途: 操作授权              │
│   ├── 有效期: 15 分钟            ├── 有效期: 长期                │
│   ├── 存储: 内存/Cookie          ├── 存储: 设备本地              │
│   ├── 刷新: 通过 Refresh Token   ├── 刷新: 通过现有轮转机制      │
│   └── 包含: userId               └── 包含: role, scopes          │
│                                                                 │
│   操作请求时:                                                    │
│   1. 验证 Device Token → 确认设备有权限执行操作                  │
│   2. 可选验证 User Token → 确认用户身份（用于审计）              │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 1.4 技术选型

| 组件 | 选型 | 说明 |
|------|------|------|
| 数据库 | PostgreSQL 16 | 成熟稳定，支持 JSON、全文搜索 |
| ORM | Drizzle ORM | 类型安全，轻量级 |
| 迁移工具 | Drizzle Kit | 内置迁移管理 |
| 连接池 | pg-pool | PostgreSQL 官方连接池 |
| 参数验证 | Zod | 类型安全的 schema 验证 |
| 密码哈希 | bcrypt (cost=12) | 行业标准 |

---

## 2. 数据库设计

### 2.1 ER 关系图（修订版）

```
┌─────────────────┐       ┌─────────────────┐       ┌─────────────────┐
│      users      │       │  user_devices   │       │  user_sessions  │
├─────────────────┤       ├─────────────────┤       ├─────────────────┤
│ id (PK)         │◄──────│ user_id (FK)    │       │ id (PK)         │
│ phone           │       │ id (PK)         │       │ user_id (FK)    │◄─┐
│ email           │       │ device_id       │───────│ refresh_token   │  │
│ password_hash   │       │ display_name    │  ▲    │ expires_at      │  │
│ display_name    │       │ linked_at       │  │    │ revoked_at      │  │
│ status          │       └─────────────────┘  │    │ created_at      │  │
│ is_verified     │                            │    └─────────────────┘  │
│ created_at      │       ┌─────────────────┐  │                         │
│ updated_at      │       │ device-pairing  │  │                         │
└────────┬────────┘       │ (现有 JSON 存储) │──┘                         │
         │                ├─────────────────┤                            │
         │                │ deviceId        │                            │
         │                │ publicKey       │                            │
         │                │ role            │                            │
         │                │ scopes          │                            │
         │                │ tokens          │                            │
         │                └─────────────────┘                            │
         │                                                               │
         │  ┌─────────────────┐       ┌─────────────────┐               │
         │  │  subscriptions  │       │   audit_logs    │               │
         │  ├─────────────────┤       ├─────────────────┤               │
         └──│ user_id (FK)    │       │ id (PK)         │───────────────┘
            │ id (PK)         │       │ user_id (FK)    │
            │ plan_id (FK)    │       │ device_id       │
            │ skill_id (FK)   │       │ action          │
            │ status          │       │ resource        │
            │ started_at      │       │ details (JSONB) │
            │ expires_at      │       │ ip_address      │
            └────────┬────────┘       │ created_at      │
                     │                └─────────────────┘
         ┌───────────┴───────────┐
         ▼                       ▼
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│      plans      │     │     skills      │     │  user_skills    │
├─────────────────┤     ├─────────────────┤     ├─────────────────┤
│ id (PK)         │     │ id (PK)         │     │ id (PK)         │
│ name            │     │ name            │     │ user_id (FK)    │
│ slug            │     │ slug            │     │ skill_id (FK)   │
│ price           │     │ price           │     │ installed_at    │
│ period          │     │ type            │     │ configuration   │
│ features (JSONB)│     │ status          │     │ is_favorite     │
│ limits (JSONB)  │     │ is_active       │     └─────────────────┘
└─────────────────┘     └─────────────────┘
```

### 2.2 用户表 (users) - 修订版

```sql
CREATE TABLE users (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),

    -- 认证信息
    phone           VARCHAR(20) UNIQUE,           -- 手机号 (E.164 格式)
    email           VARCHAR(255) UNIQUE,          -- 邮箱
    password_hash   VARCHAR(255),                 -- bcrypt 哈希 (cost=12)

    -- 第三方登录
    wechat_openid   VARCHAR(100) UNIQUE,          -- 微信 OpenID
    wechat_unionid  VARCHAR(100),                 -- 微信 UnionID

    -- 用户资料
    display_name    VARCHAR(100) NOT NULL,        -- 显示名称
    avatar_url      VARCHAR(500),                 -- 头像 URL

    -- 🆕 验证状态
    is_phone_verified BOOLEAN DEFAULT FALSE,      -- 手机号已验证
    is_email_verified BOOLEAN DEFAULT FALSE,      -- 邮箱已验证

    -- 🆕 国际化
    timezone        VARCHAR(50) DEFAULT 'Asia/Shanghai',
    locale          VARCHAR(10) DEFAULT 'zh-CN',

    -- 账户状态
    status          VARCHAR(20) NOT NULL DEFAULT 'active',
    -- 可选值: 'active' | 'suspended' | 'deleted'
    account_type    VARCHAR(20) DEFAULT 'personal', -- personal | business

    -- 🆕 MFA 增强
    mfa_enabled     BOOLEAN DEFAULT FALSE,
    mfa_method      VARCHAR(20),                  -- totp | sms | email
    mfa_secret      VARCHAR(255),                 -- 加密存储的 TOTP 密钥
    mfa_backup_codes TEXT[],                      -- 备用码数组

    -- 安全相关
    login_attempts  INTEGER DEFAULT 0,            -- 登录失败次数
    locked_until    TIMESTAMP WITH TIME ZONE,     -- 锁定截止时间
    password_changed_at TIMESTAMP WITH TIME ZONE, -- 密码最后修改时间

    -- 时间戳
    created_at      TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at      TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    last_login_at   TIMESTAMP WITH TIME ZONE,
    deleted_at      TIMESTAMP WITH TIME ZONE,     -- 🆕 软删除

    -- 约束：至少有一种登录方式
    CONSTRAINT chk_login_method CHECK (
        phone IS NOT NULL OR email IS NOT NULL OR wechat_openid IS NOT NULL
    )
);

-- 索引
CREATE INDEX idx_users_phone ON users(phone) WHERE phone IS NOT NULL;
CREATE INDEX idx_users_email ON users(email) WHERE email IS NOT NULL;
CREATE INDEX idx_users_wechat ON users(wechat_openid) WHERE wechat_openid IS NOT NULL;
CREATE INDEX idx_users_status ON users(status) WHERE deleted_at IS NULL;
```

**修订说明**：
- ❌ 移除 `role` 和 `scopes`（这些属于设备）
- ✅ 新增 `is_phone_verified`、`is_email_verified` 验证状态
- ✅ 新增 `timezone`、`locale` 国际化支持
- ✅ 新增完整的 MFA 字段（`mfa_method`、`mfa_secret`、`mfa_backup_codes`）
- ✅ 新增 `deleted_at` 支持软删除
- ✅ 新增 `password_changed_at` 安全追踪

### 2.3 用户设备关联表 (user_devices) - 修订版

```sql
-- 🆕 用户设备关联表（关联到现有 device-pairing，而非重复存储）
CREATE TABLE user_devices (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id         UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,

    -- 关联到现有 device-pairing.ts 中的设备
    device_id       VARCHAR(100) NOT NULL,        -- 对应 PairedDevice.deviceId

    -- 用户自定义信息
    display_name    VARCHAR(100),                 -- 用户给设备的昵称
    is_primary      BOOLEAN DEFAULT FALSE,        -- 是否为主设备

    -- 时间戳
    linked_at       TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    last_used_at    TIMESTAMP WITH TIME ZONE,

    -- 唯一约束
    CONSTRAINT uq_user_device UNIQUE(user_id, device_id)
);

-- 索引
CREATE INDEX idx_user_devices_user_id ON user_devices(user_id);
CREATE INDEX idx_user_devices_device_id ON user_devices(device_id);
```

**修订说明**：
- ❌ 移除 `public_key`、`fingerprint`、`role`、`scopes`、`token_hash`（这些在现有 device-pairing.ts 中管理）
- ✅ 简化为纯关联表，只存储用户与设备的绑定关系
- ✅ 新增 `is_primary` 标识主设备

### 2.4 用户会话表 (user_sessions)

```sql
-- 用户登录会话表（存储 Refresh Token）
CREATE TABLE user_sessions (
    id                  UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id             UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,

    -- Token 信息
    refresh_token_hash  VARCHAR(64) NOT NULL,     -- SHA256(refresh_token)

    -- 🆕 关联设备（可选，指向 user_devices）
    user_device_id      UUID REFERENCES user_devices(id) ON DELETE SET NULL,

    -- 来源信息
    ip_address          VARCHAR(45),              -- IPv6 最长 45 字符
    user_agent          VARCHAR(500),

    -- 有效期
    expires_at          TIMESTAMP WITH TIME ZONE NOT NULL,
    revoked_at          TIMESTAMP WITH TIME ZONE,
    last_used_at        TIMESTAMP WITH TIME ZONE,

    created_at          TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 索引
CREATE INDEX idx_user_sessions_user_id ON user_sessions(user_id);
CREATE INDEX idx_user_sessions_token ON user_sessions(refresh_token_hash);
CREATE INDEX idx_user_sessions_expires ON user_sessions(expires_at)
    WHERE revoked_at IS NULL;
```

### 2.5 登录尝试记录表 (login_attempts) - 新增

```sql
-- 🆕 登录尝试记录表（防暴力破解）
CREATE TABLE login_attempts (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),

    -- 目标（用户可能不存在）
    user_id         UUID REFERENCES users(id) ON DELETE SET NULL,
    identifier      VARCHAR(255) NOT NULL,        -- 手机号/邮箱

    -- 尝试信息
    ip_address      VARCHAR(45) NOT NULL,
    user_agent      VARCHAR(500),
    success         BOOLEAN NOT NULL,
    failure_reason  VARCHAR(50),                  -- invalid_password | user_not_found | locked

    attempted_at    TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 索引（用于查询近期失败次数）
CREATE INDEX idx_login_attempts_user_time
    ON login_attempts(user_id, attempted_at DESC)
    WHERE success = false;

CREATE INDEX idx_login_attempts_ip_time
    ON login_attempts(ip_address, attempted_at DESC);

-- 🆕 自动清理 30 天前的记录
-- 通过定时任务执行: DELETE FROM login_attempts WHERE attempted_at < NOW() - INTERVAL '30 days'
```

### 2.6 验证码表 (verification_codes) - 修订版

```sql
CREATE TABLE verification_codes (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),

    -- 目标
    target          VARCHAR(255) NOT NULL,        -- 手机号或邮箱
    target_type     VARCHAR(20) NOT NULL,         -- phone | email

    -- 验证码
    code            VARCHAR(10) NOT NULL,
    purpose         VARCHAR(20) NOT NULL,         -- register | login | reset_password | bind

    -- 状态
    verified_at     TIMESTAMP WITH TIME ZONE,
    attempts        INTEGER DEFAULT 0,
    max_attempts    INTEGER DEFAULT 5,            -- 🆕 最大尝试次数

    -- 🆕 防暴力破解
    ip_address      VARCHAR(45),
    user_agent      VARCHAR(500),

    -- 有效期
    expires_at      TIMESTAMP WITH TIME ZONE NOT NULL,
    created_at      TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 索引（优化活跃验证码查询）
CREATE INDEX idx_verification_codes_active
    ON verification_codes(target, target_type, purpose)
    WHERE verified_at IS NULL AND expires_at > NOW();

CREATE INDEX idx_verification_codes_expires
    ON verification_codes(expires_at);

-- 🆕 防止同一 IP 频繁请求
CREATE INDEX idx_verification_codes_ip
    ON verification_codes(ip_address, created_at DESC);
```

**修订说明**：
- ✅ 新增 `max_attempts` 最大尝试次数
- ✅ 新增 `ip_address`、`user_agent` 追踪
- ✅ 优化索引，支持活跃验证码快速查询

### 2.7 订阅计划表 (plans)

```sql
CREATE TABLE plans (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),

    -- 基本信息
    name            VARCHAR(100) NOT NULL,        -- 计划名称
    slug            VARCHAR(50) NOT NULL UNIQUE,  -- URL 友好标识
    description     TEXT,                         -- 描述

    -- 价格
    price           DECIMAL(10, 2) NOT NULL,      -- 价格
    currency        VARCHAR(3) DEFAULT 'CNY',     -- 货币
    period          VARCHAR(20) NOT NULL,         -- monthly | yearly | lifetime

    -- 功能配置
    features        JSONB NOT NULL DEFAULT '{}',  -- 功能特性
    limits          JSONB NOT NULL DEFAULT '{}',  -- 使用限制

    -- 状态
    is_active       BOOLEAN DEFAULT TRUE,
    display_order   INTEGER DEFAULT 0,

    created_at      TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at      TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 预置计划
INSERT INTO plans (name, slug, price, period, features, limits) VALUES
('免费版', 'free', 0, 'lifetime',
 '{"basic_skills": true}',
 '{"devices": 1, "skills": 5, "daily_requests": 100}'),
('专业版', 'pro', 29.00, 'monthly',
 '{"basic_skills": true, "advanced_skills": true, "priority_support": true}',
 '{"devices": 3, "skills": 50, "daily_requests": 1000}'),
('团队版', 'team', 99.00, 'monthly',
 '{"basic_skills": true, "advanced_skills": true, "team_features": true}',
 '{"devices": 10, "skills": -1, "daily_requests": -1}');
```

### 2.8 技能表 (skills)

```sql
CREATE TABLE skills (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),

    -- 基本信息
    name            VARCHAR(100) NOT NULL,
    slug            VARCHAR(50) NOT NULL UNIQUE,
    description     TEXT,
    version         VARCHAR(20) NOT NULL,

    -- 分类
    type            VARCHAR(20) NOT NULL,         -- server | local
    category        VARCHAR(50),
    tags            TEXT[] DEFAULT '{}',

    -- 价格（NULL = 免费）
    price           DECIMAL(10, 2),
    subscription_only BOOLEAN DEFAULT FALSE,      -- 是否仅订阅可用

    -- 技能代码
    code_url        VARCHAR(500),
    code_hash       VARCHAR(64),

    -- 权限要求
    required_permissions TEXT[] DEFAULT '{}',

    -- 统计
    install_count   INTEGER DEFAULT 0,
    rating          DECIMAL(2, 1),                -- 1.0-5.0

    -- 状态
    is_active       BOOLEAN DEFAULT TRUE,
    is_featured     BOOLEAN DEFAULT FALSE,

    created_at      TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at      TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 索引
CREATE INDEX idx_skills_slug ON skills(slug);
CREATE INDEX idx_skills_type ON skills(type);
CREATE INDEX idx_skills_category ON skills(category);
CREATE INDEX idx_skills_tags ON skills USING GIN(tags);
```

### 2.9 用户技能关联表 (user_skills) - 新增

```sql
-- 🆕 用户安装的技能
CREATE TABLE user_skills (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id         UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    skill_id        UUID NOT NULL REFERENCES skills(id) ON DELETE CASCADE,

    -- 安装信息
    installed_at    TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    configuration   JSONB DEFAULT '{}',           -- 用户自定义配置

    -- 用户偏好
    is_favorite     BOOLEAN DEFAULT FALSE,
    usage_count     INTEGER DEFAULT 0,
    last_used_at    TIMESTAMP WITH TIME ZONE,

    -- 唯一约束
    CONSTRAINT uq_user_skill UNIQUE(user_id, skill_id)
);

-- 索引
CREATE INDEX idx_user_skills_user_id ON user_skills(user_id);
CREATE INDEX idx_user_skills_skill_id ON user_skills(skill_id);
```

### 2.10 订阅表 (subscriptions) - 修订版

```sql
CREATE TABLE subscriptions (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id         UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,

    -- 订阅类型（二选一）
    plan_id         UUID REFERENCES plans(id),    -- 订阅计划
    skill_id        UUID REFERENCES skills(id),   -- 单独订阅技能

    -- 状态
    status          VARCHAR(20) NOT NULL DEFAULT 'active',
    -- 可选值: 'active' | 'expired' | 'cancelled' | 'suspended' | 'past_due'

    -- 时间
    started_at      TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    expires_at      TIMESTAMP WITH TIME ZONE,     -- NULL = 永不过期
    cancelled_at    TIMESTAMP WITH TIME ZONE,

    -- 🆕 自动续费增强
    auto_renew      BOOLEAN DEFAULT TRUE,
    current_period_end TIMESTAMP WITH TIME ZONE,  -- 当前周期结束时间
    next_billing_at TIMESTAMP WITH TIME ZONE,     -- 下次扣费时间
    failed_renew_attempts INTEGER DEFAULT 0,      -- 续费失败次数

    -- 支付信息
    payment_provider VARCHAR(50),                 -- wechat | alipay | stripe
    payment_id      VARCHAR(100),
    amount_paid     DECIMAL(10, 2),

    created_at      TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at      TIMESTAMP WITH TIME ZONE DEFAULT NOW(),

    -- 约束：必须订阅计划或技能之一
    CONSTRAINT chk_subscription_target CHECK (
        (plan_id IS NOT NULL AND skill_id IS NULL) OR
        (plan_id IS NULL AND skill_id IS NOT NULL)
    ),
    -- 🆕 日期约束
    CONSTRAINT chk_dates CHECK (
        cancelled_at IS NULL OR cancelled_at >= started_at
    ),
    CONSTRAINT chk_expires_after_started CHECK (
        expires_at IS NULL OR expires_at > started_at
    )
);

-- 索引
CREATE INDEX idx_subscriptions_user_id ON subscriptions(user_id);
CREATE INDEX idx_subscriptions_status ON subscriptions(status);
CREATE INDEX idx_subscriptions_expires ON subscriptions(expires_at)
    WHERE status = 'active';
-- 🆕 自动续费索引
CREATE INDEX idx_subscriptions_next_billing
    ON subscriptions(next_billing_at)
    WHERE auto_renew = true AND status = 'active';
```

### 2.11 支付订单表 (payment_orders) - 新增

```sql
-- 🆕 支付订单表
CREATE TABLE payment_orders (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id         UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,

    -- 订单信息
    order_no        VARCHAR(50) NOT NULL UNIQUE,  -- 订单号
    order_type      VARCHAR(20) NOT NULL,         -- subscription | skill | recharge

    -- 关联
    subscription_id UUID REFERENCES subscriptions(id),
    skill_id        UUID REFERENCES skills(id),
    plan_id         UUID REFERENCES plans(id),

    -- 金额
    amount          DECIMAL(10, 2) NOT NULL,
    currency        VARCHAR(3) DEFAULT 'CNY',
    discount_amount DECIMAL(10, 2) DEFAULT 0,
    final_amount    DECIMAL(10, 2) NOT NULL,

    -- 优惠券
    coupon_id       UUID,
    coupon_code     VARCHAR(50),

    -- 支付
    payment_provider VARCHAR(50),                 -- wechat | alipay
    payment_id      VARCHAR(100),                 -- 第三方支付订单号
    payment_url     VARCHAR(500),                 -- 支付链接
    qr_code         TEXT,                         -- 支付二维码

    -- 状态
    status          VARCHAR(20) NOT NULL DEFAULT 'pending',
    -- pending | paid | failed | cancelled | refunded
    paid_at         TIMESTAMP WITH TIME ZONE,
    failed_at       TIMESTAMP WITH TIME ZONE,
    failure_reason  TEXT,

    -- 时间
    expires_at      TIMESTAMP WITH TIME ZONE,     -- 订单过期时间
    created_at      TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at      TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 索引
CREATE INDEX idx_payment_orders_user_id ON payment_orders(user_id);
CREATE INDEX idx_payment_orders_order_no ON payment_orders(order_no);
CREATE INDEX idx_payment_orders_status ON payment_orders(status);
```

### 2.12 优惠券表 (coupon_codes) - 新增

```sql
-- 🆕 优惠券表
CREATE TABLE coupon_codes (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    code            VARCHAR(50) NOT NULL UNIQUE,

    -- 类型
    type            VARCHAR(20) NOT NULL,         -- percentage | fixed_amount | free_trial
    value           DECIMAL(10, 2) NOT NULL,      -- 折扣值或金额

    -- 适用范围
    applicable_to   VARCHAR(20) DEFAULT 'all',    -- all | plans | skills
    applicable_ids  UUID[],                       -- 如果有限制，指定适用的 ID

    -- 使用限制
    usage_limit     INTEGER,                      -- 总使用次数限制（NULL = 无限制）
    usage_per_user  INTEGER DEFAULT 1,            -- 每个用户最多使用次数
    minimum_amount  DECIMAL(10, 2),               -- 最小消费金额

    -- 有效期
    starts_at       TIMESTAMP WITH TIME ZONE NOT NULL,
    expires_at      TIMESTAMP WITH TIME ZONE NOT NULL,

    -- 状态
    is_active       BOOLEAN DEFAULT TRUE,
    created_at      TIMESTAMP WITH TIME ZONE DEFAULT NOW(),

    CONSTRAINT chk_valid_period CHECK (starts_at < expires_at),
    CONSTRAINT chk_positive_value CHECK (value > 0)
);

-- 🆕 优惠券使用记录
CREATE TABLE coupon_usage (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    coupon_id       UUID NOT NULL REFERENCES coupon_codes(id),
    user_id         UUID NOT NULL REFERENCES users(id),
    order_id        UUID REFERENCES payment_orders(id),
    discount_amount DECIMAL(10, 2),
    applied_at      TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

CREATE INDEX idx_coupon_usage_coupon_id ON coupon_usage(coupon_id);
CREATE INDEX idx_coupon_usage_user_id ON coupon_usage(user_id);
```

### 2.13 审计日志表 (audit_logs)

```sql
-- 审计日志表（按月分区）
CREATE TABLE audit_logs (
    id              UUID DEFAULT gen_random_uuid(),

    -- 主体
    user_id         UUID REFERENCES users(id) ON DELETE SET NULL,
    device_id       VARCHAR(100),

    -- 操作信息
    action          VARCHAR(50) NOT NULL,         -- 操作类型
    resource_type   VARCHAR(50) NOT NULL,         -- 资源类型
    resource_id     VARCHAR(100),                 -- 资源 ID

    -- 详情
    details         JSONB DEFAULT '{}',

    -- 结果
    status          VARCHAR(20) NOT NULL,         -- success | failure | denied
    error_message   TEXT,

    -- 来源
    ip_address      VARCHAR(45),
    user_agent      VARCHAR(500),
    request_id      VARCHAR(50),

    -- 🆕 风险评估
    risk_level      VARCHAR(20) DEFAULT 'low',    -- low | medium | high | critical

    -- 时间戳
    created_at      TIMESTAMP WITH TIME ZONE DEFAULT NOW(),

    PRIMARY KEY (id, created_at)
) PARTITION BY RANGE (created_at);

-- 创建分区（每月一个分区）
CREATE TABLE audit_logs_2026_02 PARTITION OF audit_logs
    FOR VALUES FROM ('2026-02-01') TO ('2026-03-01');

CREATE TABLE audit_logs_2026_03 PARTITION OF audit_logs
    FOR VALUES FROM ('2026-03-01') TO ('2026-04-01');

-- 索引
CREATE INDEX idx_audit_logs_user_id ON audit_logs(user_id);
CREATE INDEX idx_audit_logs_action ON audit_logs(action);
CREATE INDEX idx_audit_logs_created_at ON audit_logs(created_at);
CREATE INDEX idx_audit_logs_risk_level ON audit_logs(risk_level)
    WHERE risk_level IN ('high', 'critical');
```

### 2.14 导出记录表 (export_logs) - 新增

```sql
-- 🆕 导出记录表（用于频率限制）
CREATE TABLE export_logs (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id         UUID NOT NULL REFERENCES users(id),
    export_type     VARCHAR(50) NOT NULL,         -- audit_logs | subscription_history
    row_count       INTEGER,
    export_size_bytes INTEGER,
    ip_address      VARCHAR(45),
    created_at      TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

CREATE INDEX idx_export_logs_user_time
    ON export_logs(user_id, created_at DESC);
```

---

## 3. 认证流程设计

### 3.1 支持的认证方式

| 方式 | 说明 | 优先级 | Token 类型 |
|------|------|--------|------------|
| 手机号 + 验证码 | 主要方式，中国用户首选 | P0 | User Token |
| 邮箱 + 密码 | 国际用户、企业用户 | P0 | User Token |
| 微信扫码 | 快速登录，社交绑定 | P1 | User Token |
| 设备配对 | 现有机制，用于操作授权 | P0 (复用) | Device Token |

### 3.2 手机号注册流程（含防暴力破解）

```
     客户端                    Gateway                     数据库
        │                         │                          │
        │── 1. 请求验证码 ───────→│                          │
        │   { phone, purpose:     │                          │
        │     'register' }        │                          │
        │                         │                          │
        │                         │── 2. 检查 IP 频率限制 ──→│
        │                         │   SELECT COUNT(*) FROM   │
        │                         │   verification_codes     │
        │                         │   WHERE ip = ? AND       │
        │                         │   created_at > NOW()-1h  │
        │                         │←── count < 10 ──────────│
        │                         │                          │
        │                         │── 3. 检查手机号 ─────────→│
        │                         │   SELECT FROM users      │
        │                         │   WHERE phone = ?        │
        │                         │←── 未注册 ───────────────│
        │                         │                          │
        │                         │── 4. 检查手机号频率 ────→│
        │                         │   同一手机号 60s 内      │
        │                         │   只能请求一次           │
        │                         │                          │
        │                         │── 5. 生成验证码 ─────────→│
        │                         │   INSERT verification_   │
        │                         │   codes (6位, 5分钟有效,  │
        │                         │   max_attempts=5)        │
        │                         │                          │
        │                         │── 6. 发送短信 ──────────→│ [SMS服务]
        │                         │                          │
        │←── 验证码已发送 ────────│                          │
        │   (隐藏真实验证码)       │                          │
        │                         │                          │
        │── 7. 提交注册 ─────────→│                          │
        │   { phone, code,        │                          │
        │     display_name? }     │                          │
        │                         │                          │
        │                         │── 8. 验证码校验 ─────────→│
        │                         │   WHERE target=phone     │
        │                         │   AND code=? AND         │
        │                         │   expires_at > NOW() AND │
        │                         │   attempts < max_attempts│
        │                         │                          │
        │                         │   [验证失败]             │
        │                         │── 8a. 增加尝试次数 ─────→│
        │                         │   UPDATE attempts += 1   │
        │←── 验证码错误 ──────────│                          │
        │                         │                          │
        │                         │   [验证成功]             │
        │                         │── 9. 创建用户 ───────────→│
        │                         │   INSERT users           │
        │                         │   (is_phone_verified=true)│
        │                         │←── 用户创建成功 ─────────│
        │                         │                          │
        │                         │── 10. 生成 Token ────────│
        │                         │   User Access Token (15m)│
        │                         │   Refresh Token (7d)     │
        │                         │                          │
        │                         │── 11. 存储会话 ──────────→│
        │                         │   INSERT user_sessions   │
        │                         │                          │
        │                         │── 12. 审计日志 ──────────→│
        │                         │   action: 'user.register'│
        │                         │                          │
        │←── 注册成功 ────────────│                          │
        │   { accessToken,        │                          │
        │     refreshToken,       │                          │
        │     expiresIn,          │                          │
        │     user }              │                          │
```

### 3.3 登录流程（含账户锁定）

```
     客户端                    Gateway                     数据库
        │                         │                          │
        │── 1. 登录请求 ─────────→│                          │
        │   { phone/email,        │                          │
        │     password/code }     │                          │
        │                         │                          │
        │                         │── 2. 查询用户 ───────────→│
        │                         │                          │
        │                         │   [用户不存在]           │
        │                         │── 2a. 记录尝试 ──────────→│
        │                         │   INSERT login_attempts  │
        │                         │   (user_id=NULL,         │
        │                         │    success=false)        │
        │←── 登录失败 ────────────│                          │
        │   (不泄露用户存在信息)   │                          │
        │                         │                          │
        │                         │←── 用户信息 ─────────────│
        │                         │                          │
        │                         │── 3. 检查账户状态 ───────│
        │                         │   status = 'active'?     │
        │                         │   locked_until < NOW()?  │
        │                         │                          │
        │                         │   [账户被锁定]           │
        │←── 账户已锁定 ──────────│                          │
        │   (显示剩余锁定时间)     │                          │
        │                         │                          │
        │                         │── 4. 验证凭据 ───────────│
        │                         │   bcrypt.compare() 或    │
        │                         │   验证码校验             │
        │                         │                          │
        │                         │   [验证失败]             │
        │                         │── 5a. 记录失败 ─────────→│
        │                         │   INSERT login_attempts  │
        │                         │   (success=false)        │
        │                         │                          │
        │                         │── 5b. 检查失败次数 ─────→│
        │                         │   SELECT COUNT(*) FROM   │
        │                         │   login_attempts WHERE   │
        │                         │   user_id=? AND          │
        │                         │   success=false AND      │
        │                         │   attempted_at > -15min  │
        │                         │                          │
        │                         │   [>=5次失败]            │
        │                         │── 5c. 锁定账户 ─────────→│
        │                         │   UPDATE users SET       │
        │                         │   locked_until = +15min  │
        │                         │                          │
        │                         │── 审计日志 ─────────────→│
        │                         │   action: 'user.login'   │
        │                         │   status: 'failure'      │
        │                         │   risk_level: 'medium'   │
        │←── 登录失败 ────────────│                          │
        │                         │                          │
        │                         │   [验证成功]             │
        │                         │── 6. 重置失败次数 ──────→│
        │                         │   UPDATE users SET       │
        │                         │   login_attempts = 0,    │
        │                         │   locked_until = NULL,   │
        │                         │   last_login_at = NOW()  │
        │                         │                          │
        │                         │── 7. 检查 MFA ───────────│
        │                         │   mfa_enabled = true?    │
        │                         │                          │
        │                         │   [需要 MFA]             │
        │←── 需要二次验证 ────────│                          │
        │   { mfaRequired: true,  │                          │
        │     mfaMethod: 'totp' } │                          │
        │                         │                          │
        │── 8. 提交 MFA ─────────→│                          │
        │   { mfaCode }           │                          │
        │                         │── 9. 验证 MFA ───────────│
        │                         │   speakeasy.verify()     │
        │                         │                          │
        │                         │── 10. 生成 Token ────────│
        │                         │                          │
        │                         │── 11. 审计日志 ─────────→│
        │                         │   action: 'user.login'   │
        │                         │   status: 'success'      │
        │                         │                          │
        │←── 登录成功 ────────────│                          │
        │   { accessToken,        │                          │
        │     refreshToken,       │                          │
        │     user }              │                          │
```

### 3.4 Token 设计（修订版）

#### User Access Token 结构

```typescript
// User Access Token - 仅用于身份验证
interface UserAccessTokenPayload {
  sub: string           // 用户 ID
  type: 'user'          // Token 类型
  aud: 'openclaw-api'   // 受众
  iat: number           // 签发时间
  exp: number           // 过期时间 (15分钟)
  // ❌ 不包含 role 和 scopes（这些属于设备）
}
```

#### Refresh Token 设计

```typescript
// Refresh Token - 存储在数据库，仅哈希值
interface RefreshTokenRecord {
  id: string              // UUID
  userId: string          // 用户 ID
  tokenHash: string       // SHA256(token)
  expiresAt: Date         // 7 天有效期
  revokedAt?: Date        // 撤销时间
  lastUsedAt?: Date       // 最后使用时间
  ipAddress?: string      // 创建时的 IP
  userAgent?: string      // 创建时的 UA
}

// 刷新策略
const REFRESH_CONFIG = {
  expiresIn: '7d',        // 7天有效期
  rotateThreshold: 2,     // 剩余2天时自动轮转
  maxSessionsPerUser: 5,  // 每个用户最多 5 个活跃会话
}
```

#### Token 生成实现

```typescript
// src/assistant/auth/jwt.ts
import jwt from 'jsonwebtoken'
import crypto from 'crypto'

/**
 * 生成 User Token 对（Access + Refresh）
 */
export async function generateUserTokenPair(
  user: User,
  ctx: GatewayContext
): Promise<{ accessToken: string; refreshToken: string; expiresIn: number }> {
  // 1. 生成 Access Token (JWT)
  const accessToken = jwt.sign(
    {
      sub: user.id,
      type: 'user',
      aud: 'openclaw-api',
    },
    ctx.config.jwtSecret,
    {
      expiresIn: '15m',
      issuer: 'openclaw-gateway',
    }
  )

  // 2. 生成 Refresh Token（随机字符串）
  const refreshToken = crypto.randomBytes(32).toString('base64url')
  const refreshTokenHash = crypto.createHash('sha256')
    .update(refreshToken).digest('hex')

  // 3. 清理过期会话，保持最多 5 个活跃会话
  await cleanupUserSessions(user.id, ctx)

  // 4. 存储到数据库
  await ctx.db.insert(userSessions).values({
    user_id: user.id,
    refresh_token_hash: refreshTokenHash,
    expires_at: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000),
    ip_address: ctx.req?.ip,
    user_agent: ctx.req?.headers?.['user-agent'],
  })

  return {
    accessToken,
    refreshToken,
    expiresIn: 15 * 60, // 秒
  }
}

/**
 * 清理用户过期会话，保持最多 N 个活跃会话
 */
async function cleanupUserSessions(userId: string, ctx: GatewayContext) {
  // 删除过期会话
  await ctx.db.delete(userSessions)
    .where(and(
      eq(userSessions.user_id, userId),
      lt(userSessions.expires_at, new Date())
    ))

  // 获取活跃会话数
  const activeSessions = await ctx.db.query.userSessions.findMany({
    where: and(
      eq(userSessions.user_id, userId),
      isNull(userSessions.revoked_at),
      gt(userSessions.expires_at, new Date())
    ),
    orderBy: [desc(userSessions.created_at)],
  })

  // 如果超过限制，撤销最旧的会话
  if (activeSessions.length >= REFRESH_CONFIG.maxSessionsPerUser) {
    const toRevoke = activeSessions.slice(REFRESH_CONFIG.maxSessionsPerUser - 1)
    for (const session of toRevoke) {
      await ctx.db.update(userSessions)
        .set({ revoked_at: new Date() })
        .where(eq(userSessions.id, session.id))
    }
  }
}
```

### 3.5 设备绑定流程（与现有系统整合）

```
     Windows 客户端              Gateway                     数据库/现有系统
        │                         │                          │
        │── 1. 用户已登录 ────────│                          │
        │   (持有 User Token)     │                          │
        │                         │                          │
        │── 2. 设备配对请求 ─────→│                          │
        │   Authorization: Bearer │                          │
        │   X-Device-Id: xxx      │                          │
        │   { publicKey,          │                          │
        │     signature,          │                          │
        │     platform }          │                          │
        │                         │                          │
        │                         │── 3. 验证 User Token ────│
        │                         │                          │
        │                         │── 4. 验证设备签名 ───────│
        │                         │   Ed25519.verify()       │
        │                         │                          │
        │                         │── 5. 检查设备配额 ───────→│
        │                         │   SELECT COUNT(*) FROM   │
        │                         │   user_devices WHERE     │
        │                         │   user_id = ?            │
        │                         │←── 设备数: N ────────────│
        │                         │                          │
        │                         │   [超出配额]             │
        │←── 错误: 设备已达上限 ──│                          │
        │                         │                          │
        │                         │   [配额内]               │
        │                         │── 6. 调用现有配对 ───────→│ device-pairing.ts
        │                         │   devicePairing.         │
        │                         │     createPairedDevice() │
        │                         │←── Device Token ─────────│
        │                         │                          │
        │                         │── 7. 创建关联记录 ───────→│
        │                         │   INSERT user_devices    │
        │                         │   (user_id, device_id)   │
        │                         │                          │
        │                         │── 8. 审计日志 ───────────→│
        │                         │   action: 'device.linked'│
        │                         │                          │
        │←── 绑定成功 ────────────│                          │
        │   { deviceToken,        │                          │
        │     role, scopes }      │                          │
```

**关键点**：
- 设备配对仍然使用现有的 `device-pairing.ts` 机制
- 新增 `user_devices` 表只存储用户与设备的关联关系
- 设备的 role、scopes、token 仍由现有系统管理

## 4. 订阅与支付系统

### 4.1 订阅模型

```
                    ┌─────────────┐
                    │    用户     │
                    └──────┬──────┘
                           │
              ┌────────────┼────────────┐
              ▼            │            ▼
       ┌──────────┐        │      ┌──────────┐
       │ 计划订阅  │        │      │ 技能订阅  │
       │ (Plan)   │        │      │ (Skill)  │
       └────┬─────┘        │      └────┬─────┘
            │              │            │
            ▼              │            ▼
     ┌──────────┐          │      ┌──────────┐
     │ 免费版   │          │      │ 单个技能  │
     │ 专业版   │          │      │ 按月付费  │
     │ 团队版   │          │      │          │
     └──────────┘          │      └──────────┘
            │              │            │
            └──────────────┼────────────┘
                           ▼
                    ┌─────────────┐
                    │  权限计算   │
                    │  - 设备数量 │
                    │  - 技能访问 │
                    │  - API 限额 │
                    └─────────────┘
```

### 4.2 订阅计划配置

| 计划 | 价格 | 设备数 | 技能数 | 每日请求 | 功能 |
|------|------|--------|--------|----------|------|
| 免费版 | ¥0 | 1 | 5 | 100 | 基础技能 |
| 专业版 | ¥29/月 | 3 | 50 | 1000 | 高级技能、优先支持 |
| 团队版 | ¥99/月 | 10 | 无限 | 无限 | 团队协作、API 访问 |

### 4.3 购买订阅流程（修订版）

```
     客户端                    Gateway                     数据库/支付
        │                         │                          │
        │── 1. 获取计划列表 ─────→│                          │
        │   GET /plans            │                          │
        │                         │── 查询计划 ──────────────→│
        │←── 计划列表 ────────────│                          │
        │                         │                          │
        │── 2. 选择计划并购买 ───→│                          │
        │   POST /subscription    │                          │
        │   { planSlug: 'pro',    │                          │
        │     paymentMethod,      │                          │
        │     couponCode? }       │                          │
        │                         │                          │
        │                         │── 3. 验证用户 ────────────│
        │                         │                          │
        │                         │── 4. 验证优惠券 ─────────→│
        │                         │   (如果有)               │
        │                         │←── 折扣金额 ─────────────│
        │                         │                          │
        │                         │── 5. 创建支付订单 ───────→│
        │                         │   INSERT payment_orders  │
        │                         │                          │
        │                         │── 6. 调用支付网关 ───────→│ [微信/支付宝]
        │                         │←── 支付链接/二维码 ──────│
        │                         │                          │
        │←── 返回支付信息 ────────│                          │
        │   { orderId,            │                          │
        │     paymentUrl,         │                          │
        │     qrCode }            │                          │
        │                         │                          │
        │   [用户完成支付]         │                          │
        │                         │                          │
        │                         │←── 7. 支付回调 ──────────│ [支付网关]
        │                         │   { orderId, status }    │
        │                         │                          │
        │                         │── 8. 更新订单状态 ───────→│
        │                         │   UPDATE payment_orders  │
        │                         │   SET status = 'paid'    │
        │                         │                          │
        │                         │── 9. 创建订阅记录 ───────→│
        │                         │   INSERT subscriptions   │
        │                         │                          │
        │                         │── 10. 记录优惠券使用 ───→│
        │                         │   INSERT coupon_usage    │
        │                         │                          │
        │                         │── 11. 审计日志 ──────────→│
        │                         │   action: 'subscription. │
        │                         │     created'             │
        │                         │                          │
        │←── 12. 推送通知 ────────│                          │
        │   { subscription }      │                          │
```

### 4.4 自动续费流程（新增）

```typescript
// src/assistant/subscription/auto-renew.ts

/**
 * 自动续费定时任务（每天凌晨 2 点执行）
 */
export async function processAutoRenewals(ctx: GatewayContext): Promise<void> {
  const log = ctx.logger.child({ task: 'auto-renewal' })

  // 1. 查询需要续费的订阅
  const toRenew = await ctx.db.query.subscriptions.findMany({
    where: and(
      eq(subscriptions.auto_renew, true),
      eq(subscriptions.status, 'active'),
      lte(subscriptions.next_billing_at, new Date()),
      lt(subscriptions.failed_renew_attempts, 3) // 最多重试 3 次
    ),
    with: { user: true, plan: true },
  })

  log.info(`Found ${toRenew.length} subscriptions to renew`)

  for (const subscription of toRenew) {
    try {
      // 2. 获取用户默认支付方式
      const paymentMethod = await getDefaultPaymentMethod(subscription.user_id, ctx)

      if (!paymentMethod) {
        log.warn(`User ${subscription.user_id} has no payment method`)
        await markRenewalFailed(subscription.id, 'no_payment_method', ctx)
        continue
      }

      // 3. 创建续费订单
      const order = await createPaymentOrder({
        userId: subscription.user_id,
        orderType: 'subscription',
        subscriptionId: subscription.id,
        planId: subscription.plan_id,
        amount: subscription.plan.price,
        paymentProvider: paymentMethod.type,
      }, ctx)

      // 4. 发起自动扣款
      const result = await initiateAutoCharge(order.id, paymentMethod, ctx)

      if (result.success) {
        // 5. 更新订阅周期
        const nextPeriodEnd = calculateNextPeriodEnd(
          subscription.current_period_end,
          subscription.plan.period
        )

        await ctx.db.update(subscriptions)
          .set({
            current_period_end: nextPeriodEnd,
            next_billing_at: nextPeriodEnd,
            failed_renew_attempts: 0,
            updated_at: new Date(),
          })
          .where(eq(subscriptions.id, subscription.id))

        // 6. 审计日志
        await logAudit(ctx, {
          user_id: subscription.user_id,
          action: 'subscription.renewed',
          resource_type: 'subscription',
          resource_id: subscription.id,
          status: 'success',
        })

        log.info(`Renewed subscription ${subscription.id}`)
      } else {
        await markRenewalFailed(subscription.id, result.reason, ctx)
      }
    } catch (error) {
      log.error(`Failed to renew subscription ${subscription.id}:`, error)
      await markRenewalFailed(subscription.id, 'system_error', ctx)
    }
  }
}

/**
 * 标记续费失败
 */
async function markRenewalFailed(
  subscriptionId: string,
  reason: string,
  ctx: GatewayContext
): Promise<void> {
  const subscription = await ctx.db.query.subscriptions.findFirst({
    where: eq(subscriptions.id, subscriptionId)
  })

  const newAttempts = (subscription?.failed_renew_attempts ?? 0) + 1

  // 3 次失败后暂停订阅
  if (newAttempts >= 3) {
    await ctx.db.update(subscriptions)
      .set({
        status: 'past_due',
        auto_renew: false,
        failed_renew_attempts: newAttempts,
        updated_at: new Date(),
      })
      .where(eq(subscriptions.id, subscriptionId))

    // 通知用户
    await sendSubscriptionExpiredNotification(subscription?.user_id, ctx)
  } else {
    await ctx.db.update(subscriptions)
      .set({
        failed_renew_attempts: newAttempts,
        updated_at: new Date(),
      })
      .where(eq(subscriptions.id, subscriptionId))
  }

  await logAudit(ctx, {
    user_id: subscription?.user_id,
    action: 'subscription.renew_failed',
    resource_type: 'subscription',
    resource_id: subscriptionId,
    status: 'failure',
    details: { reason, attempts: newAttempts },
  })
}
```

### 4.5 优惠券验证逻辑（新增）

```typescript
// src/assistant/subscription/coupon.ts

/**
 * 验证并应用优惠券
 */
export async function validateAndApplyCoupon(
  couponCode: string,
  userId: string,
  planId: string,
  amount: number,
  ctx: GatewayContext
): Promise<{ valid: boolean; discountAmount: number; message?: string }> {
  // 1. 查询优惠券
  const coupon = await ctx.db.query.couponCodes.findFirst({
    where: eq(couponCodes.code, couponCode)
  })

  if (!coupon || !coupon.is_active) {
    return { valid: false, discountAmount: 0, message: '优惠券不存在或已失效' }
  }

  // 2. 检查有效期
  const now = new Date()
  if (coupon.starts_at > now || coupon.expires_at < now) {
    return { valid: false, discountAmount: 0, message: '优惠券已过期' }
  }

  // 3. 检查总使用次数
  if (coupon.usage_limit) {
    const totalUsage = await ctx.db.select({ count: count() })
      .from(couponUsage)
      .where(eq(couponUsage.coupon_id, coupon.id))

    if (totalUsage[0].count >= coupon.usage_limit) {
      return { valid: false, discountAmount: 0, message: '优惠券已用完' }
    }
  }

  // 4. 检查用户使用次数
  const userUsage = await ctx.db.select({ count: count() })
    .from(couponUsage)
    .where(and(
      eq(couponUsage.coupon_id, coupon.id),
      eq(couponUsage.user_id, userId)
    ))

  if (userUsage[0].count >= coupon.usage_per_user) {
    return { valid: false, discountAmount: 0, message: '您已达到该优惠券的使用次数限制' }
  }

  // 5. 检查适用范围
  if (coupon.applicable_to === 'plans' && coupon.applicable_ids) {
    if (!coupon.applicable_ids.includes(planId)) {
      return { valid: false, discountAmount: 0, message: '该优惠券不适用于此计划' }
    }
  }

  // 6. 检查最小消费
  if (coupon.minimum_amount && amount < Number(coupon.minimum_amount)) {
    return {
      valid: false,
      discountAmount: 0,
      message: `最小消费 ¥${coupon.minimum_amount} 才能使用此优惠券`
    }
  }

  // 7. 计算折扣
  let discountAmount = 0
  if (coupon.type === 'percentage') {
    discountAmount = amount * (Number(coupon.value) / 100)
  } else if (coupon.type === 'fixed_amount') {
    discountAmount = Math.min(Number(coupon.value), amount)
  }

  return { valid: true, discountAmount: Math.round(discountAmount * 100) / 100 }
}
```

### 4.6 订阅 RPC 方法（修订版）

```typescript
// src/gateway/server-methods/assistant-subscription.ts
import { z } from 'zod'

// 参数验证 Schema
const CreateSubscriptionParams = z.object({
  planSlug: z.string().min(1).max(50),
  paymentMethod: z.enum(['wechat', 'alipay']),
  couponCode: z.string().max(50).optional(),
})

const CancelSubscriptionParams = z.object({
  subscriptionId: z.string().uuid(),
})

export const subscriptionMethods = {
  /**
   * 获取可用计划列表
   */
  'subscription.plans.list': async ({ respond }, ctx) => {
    const planList = await ctx.db.query.plans.findMany({
      where: eq(plans.is_active, true),
      orderBy: [asc(plans.display_order)],
    })
    respond(true, planList, undefined)
  },

  /**
   * 获取用户订阅状态
   */
  'subscription.status': async ({ respond }, ctx) => {
    requireAuth(ctx)

    const [plan, userSubscriptions, usage] = await Promise.all([
      getUserActivePlan(ctx.user.id, ctx),
      getUserSubscriptions(ctx.user.id, ctx),
      getUserUsageStats(ctx.user.id, ctx),
    ])

    respond(true, { plan, subscriptions: userSubscriptions, usage }, undefined)
  },

  /**
   * 创建订阅
   */
  'subscription.create': async ({ params, respond }, ctx) => {
    requireAuth(ctx)

    // 参数验证
    const validated = CreateSubscriptionParams.parse(params)

    // 免费计划直接激活
    if (validated.planSlug === 'free') {
      const result = await activateFreePlan(ctx.user.id, ctx)
      respond(true, result, undefined)
      return
    }

    // 获取计划
    const plan = await ctx.db.query.plans.findFirst({
      where: eq(plans.slug, validated.planSlug)
    })

    if (!plan) {
      respond(false, undefined, errorShape(ErrorCodes.NOT_FOUND, '计划不存在'))
      return
    }

    // 验证优惠券
    let discountAmount = 0
    if (validated.couponCode) {
      const couponResult = await validateAndApplyCoupon(
        validated.couponCode,
        ctx.user.id,
        plan.id,
        Number(plan.price),
        ctx
      )
      if (!couponResult.valid) {
        respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, couponResult.message))
        return
      }
      discountAmount = couponResult.discountAmount
    }

    // 创建支付订单
    const order = await createPaymentOrder({
      userId: ctx.user.id,
      orderType: 'subscription',
      planId: plan.id,
      amount: Number(plan.price),
      discountAmount,
      finalAmount: Number(plan.price) - discountAmount,
      couponCode: validated.couponCode,
      paymentProvider: validated.paymentMethod,
    }, ctx)

    respond(true, {
      orderId: order.id,
      orderNo: order.order_no,
      amount: order.final_amount,
      paymentUrl: order.payment_url,
      qrCode: order.qr_code,
    }, undefined)
  },

  /**
   * 取消订阅
   */
  'subscription.cancel': async ({ params, respond }, ctx) => {
    requireAuth(ctx)

    const validated = CancelSubscriptionParams.parse(params)

    // 查询订阅
    const subscription = await ctx.db.query.subscriptions.findFirst({
      where: and(
        eq(subscriptions.id, validated.subscriptionId),
        eq(subscriptions.user_id, ctx.user.id)
      )
    })

    if (!subscription) {
      respond(false, undefined, errorShape(ErrorCodes.NOT_FOUND, '订阅不存在'))
      return
    }

    if (subscription.status !== 'active') {
      respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, '订阅已取消或已过期'))
      return
    }

    // 取消自动续费（保留到期前的使用权限）
    await ctx.db.update(subscriptions)
      .set({
        auto_renew: false,
        cancelled_at: new Date(),
        updated_at: new Date(),
      })
      .where(eq(subscriptions.id, validated.subscriptionId))

    await logAudit(ctx, {
      user_id: ctx.user.id,
      action: 'subscription.cancelled',
      resource_type: 'subscription',
      resource_id: validated.subscriptionId,
      status: 'success',
    })

    respond(true, { success: true, expiresAt: subscription.expires_at }, undefined)
  },
}
```

---

## 5. 审计日志系统

### 5.1 审计事件分类

| 类别 | 事件 | 描述 | 风险级别 |
|------|------|------|----------|
| **用户** | user.register | 用户注册 | low |
| | user.login | 用户登录 | low |
| | user.login_failed | 登录失败 | medium |
| | user.logout | 用户登出 | low |
| | user.password_change | 修改密码 | medium |
| | user.mfa_enabled | 启用 MFA | low |
| | user.mfa_disabled | 禁用 MFA | high |
| | user.profile_update | 更新资料 | low |
| **设备** | device.linked | 设备绑定 | low |
| | device.unlinked | 设备解绑 | medium |
| | device.token_refresh | Token 刷新 | low |
| **技能** | skill.execute | 技能执行 | low |
| | skill.access_denied | 权限拒绝 | medium |
| | skill.error | 执行错误 | medium |
| **文件** | file.read | 文件读取 | low |
| | file.write | 文件写入 | medium |
| | file.delete | 文件删除 | high |
| **系统** | system.command | 系统命令 | high |
| | process.kill | 结束进程 | critical |
| **订阅** | subscription.created | 创建订阅 | low |
| | subscription.cancelled | 取消订阅 | low |
| | subscription.expired | 订阅过期 | low |
| | subscription.renewed | 订阅续费 | low |
| **支付** | payment.created | 创建订单 | low |
| | payment.completed | 支付成功 | low |
| | payment.failed | 支付失败 | medium |
| | payment.refunded | 退款 | high |
| **导出** | export.audit_logs | 导出审计日志 | medium |

### 5.2 审计服务实现

```typescript
// src/assistant/audit/service.ts

export interface AuditLogInput {
  user_id?: string
  device_id?: string
  action: string
  resource_type: string
  resource_id?: string
  details?: Record<string, unknown>
  status: 'success' | 'failure' | 'denied'
  error_message?: string
  risk_level?: 'low' | 'medium' | 'high' | 'critical'
}

// 风险级别映射
const ACTION_RISK_LEVELS: Record<string, AuditLogInput['risk_level']> = {
  'user.login_failed': 'medium',
  'user.mfa_disabled': 'high',
  'device.unlinked': 'medium',
  'file.delete': 'high',
  'system.command': 'high',
  'process.kill': 'critical',
  'payment.refunded': 'high',
}

/**
 * 记录审计日志
 */
export async function logAudit(
  ctx: GatewayContext,
  log: AuditLogInput
): Promise<void> {
  // 自动推断风险级别
  const riskLevel = log.risk_level ?? ACTION_RISK_LEVELS[log.action] ?? 'low'

  try {
    await ctx.db.insert(auditLogs).values({
      ...log,
      risk_level: riskLevel,
      ip_address: ctx.req?.ip,
      user_agent: ctx.req?.headers?.['user-agent'],
      request_id: ctx.requestId,
      created_at: new Date(),
    })

    // 高风险操作发送告警
    if (riskLevel === 'critical' || riskLevel === 'high') {
      await sendSecurityAlert(log, ctx)
    }
  } catch (error) {
    // 审计日志写入失败不应影响主流程
    ctx.logger.error('[Audit] Failed to write:', error)
  }
}

/**
 * 查询审计日志（带分页）
 */
export async function queryAuditLogs(
  params: {
    userId?: string
    deviceId?: string
    action?: string
    riskLevel?: string
    startTime?: Date
    endTime?: Date
    limit?: number
    offset?: number
  },
  ctx: GatewayContext
): Promise<{ logs: AuditLog[]; total: number; hasMore: boolean }> {
  const conditions: SQL[] = []

  if (params.userId) {
    conditions.push(eq(auditLogs.user_id, params.userId))
  }
  if (params.deviceId) {
    conditions.push(eq(auditLogs.device_id, params.deviceId))
  }
  if (params.action) {
    conditions.push(eq(auditLogs.action, params.action))
  }
  if (params.riskLevel) {
    conditions.push(eq(auditLogs.risk_level, params.riskLevel))
  }
  if (params.startTime) {
    conditions.push(gte(auditLogs.created_at, params.startTime))
  }
  if (params.endTime) {
    conditions.push(lte(auditLogs.created_at, params.endTime))
  }

  const whereClause = conditions.length > 0 ? and(...conditions) : undefined
  const limit = Math.min(params.limit ?? 50, 100)
  const offset = params.offset ?? 0

  const [logs, countResult] = await Promise.all([
    ctx.db.query.auditLogs.findMany({
      where: whereClause,
      orderBy: [desc(auditLogs.created_at)],
      limit: limit + 1, // 多查一条判断是否还有更多
      offset,
    }),
    ctx.db.select({ count: count() })
      .from(auditLogs)
      .where(whereClause),
  ])

  const hasMore = logs.length > limit
  if (hasMore) logs.pop()

  return {
    logs,
    total: countResult[0].count,
    hasMore,
  }
}

/**
 * 导出审计日志（带频率限制）
 */
export async function exportAuditLogs(
  userId: string,
  startTime: Date,
  endTime: Date,
  ctx: GatewayContext
): Promise<{ data: string; format: 'csv' }> {
  // 1. 检查导出频率限制
  const oneHourAgo = new Date(Date.now() - 3600000)
  const recentExports = await ctx.db.select({ count: count() })
    .from(exportLogs)
    .where(and(
      eq(exportLogs.user_id, userId),
      gte(exportLogs.created_at, oneHourAgo)
    ))

  if (recentExports[0].count >= 3) {
    throw new Error('导出频率超限，每小时最多导出 3 次')
  }

  // 2. 限制导出范围
  const days = (endTime.getTime() - startTime.getTime()) / (24 * 60 * 60 * 1000)
  if (days > 90) {
    throw new Error('导出范围不能超过 90 天')
  }

  // 3. 查询日志
  const logs = await ctx.db.query.auditLogs.findMany({
    where: and(
      eq(auditLogs.user_id, userId),
      gte(auditLogs.created_at, startTime),
      lte(auditLogs.created_at, endTime)
    ),
    orderBy: [asc(auditLogs.created_at)],
    limit: 10000,
  })

  // 4. 生成 CSV
  const headers = ['Time', 'Action', 'Resource', 'Status', 'Risk', 'IP', 'Details']
  const rows = logs.map(log => [
    log.created_at.toISOString(),
    log.action,
    `${log.resource_type}:${log.resource_id || ''}`,
    log.status,
    log.risk_level,
    log.ip_address || '',
    JSON.stringify(log.details || {}),
  ])

  const csv = [
    headers.join(','),
    ...rows.map(row => row.map(cell => `"${String(cell).replace(/"/g, '""')}"`).join(','))
  ].join('\n')

  // 5. 记录导出
  await ctx.db.insert(exportLogs).values({
    user_id: userId,
    export_type: 'audit_logs',
    row_count: logs.length,
    export_size_bytes: Buffer.byteLength(csv, 'utf8'),
    ip_address: ctx.req?.ip,
  })

  await logAudit(ctx, {
    user_id: userId,
    action: 'export.audit_logs',
    resource_type: 'audit_logs',
    details: { rowCount: logs.length, startTime, endTime },
    status: 'success',
    risk_level: 'medium',
  })

  return { data: csv, format: 'csv' }
}
```

---

## 6. 认证机制整合

### 6.1 统一认证架构（修订版）

```
┌─────────────────────────────────────────────────────────────────────────┐
│                          认证层级架构（修订版）                           │
└─────────────────────────────────────────────────────────────────────────┘

                              ┌─────────────┐
                              │  客户端请求  │
                              └──────┬──────┘
                                     │
                    ┌────────────────┼────────────────┐
                    ▼                ▼                ▼
             ┌──────────┐     ┌──────────┐     ┌──────────┐
             │ User JWT │     │ Device   │     │ Gateway  │
             │ (身份)   │     │ Token    │     │ Token    │
             │          │     │ (权限)   │     │ (管理)   │
             └────┬─────┘     └────┬─────┘     └────┬─────┘
                  │                │                │
                  │    职责分离    │                │
                  │    ──────────  │                │
                  │    用户≠设备   │                │
                  └────────────────┼────────────────┘
                                   ▼
                            ┌─────────────┐
                            │  统一认证   │
                            │  中间件     │
                            └──────┬──────┘
                                   │
                    ┌──────────────┼──────────────┐
                    ▼              ▼              ▼
             ┌──────────┐   ┌──────────┐   ┌──────────┐
             │ 用户上下文│   │ 设备上下文│   │ 权限验证 │
             │ (可选)   │   │ (主要)   │   │ (设备)  │
             └──────────┘   └──────────┘   └──────────┘
```

### 6.2 统一认证中间件（修订版）

```typescript
// src/gateway/middleware/auth.ts

export interface AuthContext {
  // 用户信息（可选，来自 User JWT）
  user?: {
    id: string
    displayName: string
    status: string
  }

  // 设备信息（来自 Device Token 或现有系统）
  device?: {
    deviceId: string
    platform: string
    role: string
    scopes: string[]
  }

  // 认证方式
  authMethod: 'user_jwt' | 'device' | 'gateway' | 'tailscale' | 'none'

  // 有效权限（来自设备，而非用户）
  role: string
  scopes: string[]
}

/**
 * 统一认证中间件
 */
export async function authenticateRequest(
  req: GatewayRequest,
  ctx: GatewayContext
): Promise<AuthContext> {
  const authHeader = req.headers?.authorization
  const deviceId = req.headers?.['x-device-id']
  const deviceToken = req.headers?.['x-device-token']

  let authContext: AuthContext = {
    authMethod: 'none',
    role: 'anonymous',
    scopes: [],
  }

  // 1. 尝试 Device Token 认证（主要认证方式）
  if (deviceId && deviceToken) {
    const device = await verifyDeviceToken(deviceId, deviceToken, ctx)
    if (device) {
      authContext = {
        device,
        authMethod: 'device',
        role: device.role,
        scopes: device.scopes,
      }
    }
  }

  // 2. 尝试 User JWT 认证（可选，用于身份识别）
  if (authHeader?.startsWith('Bearer ')) {
    const token = authHeader.slice(7)

    // 先尝试解析为 User JWT
    const jwtPayload = await verifyUserAccessToken(token, ctx)
    if (jwtPayload) {
      const user = await ctx.db.query.users.findFirst({
        where: and(
          eq(users.id, jwtPayload.sub),
          eq(users.status, 'active')
        )
      })

      if (user) {
        authContext.user = {
          id: user.id,
          displayName: user.display_name,
          status: user.status,
        }

        // 如果没有设备认证，则使用 User JWT 作为主要认证
        if (authContext.authMethod === 'none') {
          authContext.authMethod = 'user_jwt'
          // 注意：User JWT 不提供 role/scopes，需要设备认证
        }
      }
    } else {
      // 尝试 Gateway Token（现有机制）
      const gatewayAuth = await authorizeGatewayConnect({ auth: { token } })
      if (gatewayAuth.ok) {
        authContext = {
          authMethod: 'gateway',
          role: 'operator',
          scopes: ['operator.admin'],
        }
      }
    }
  }

  // 3. 尝试 Tailscale 认证（现有机制）
  if (authContext.authMethod === 'none' && ctx.config.gateway.auth.allowTailscale) {
    const tailscaleResult = await authorizeTailscale(req)
    if (tailscaleResult.ok) {
      authContext = {
        authMethod: 'tailscale',
        role: 'operator',
        scopes: ['operator.read', 'operator.write'],
      }
    }
  }

  return authContext
}

/**
 * 要求用户认证
 */
export function requireUser(ctx: GatewayContext): void {
  if (!ctx.auth?.user) {
    throw new AuthError('User authentication required', 'USER_AUTH_REQUIRED')
  }
}

/**
 * 要求设备认证
 */
export function requireDevice(ctx: GatewayContext): void {
  if (!ctx.auth?.device) {
    throw new AuthError('Device authentication required', 'DEVICE_AUTH_REQUIRED')
  }
}

/**
 * 要求特定权限（来自设备）
 */
export function requireScope(ctx: GatewayContext, scope: string): void {
  const hasScope = ctx.auth?.scopes?.includes(scope) ||
                   ctx.auth?.scopes?.includes('operator.admin')
  if (!hasScope) {
    throw new AuthError(`Missing required scope: ${scope}`, 'SCOPE_REQUIRED')
  }
}

/**
 * 要求用户和设备都认证
 */
export function requireUserAndDevice(ctx: GatewayContext): void {
  requireUser(ctx)
  requireDevice(ctx)
}
```

### 6.3 RPC 方法权限配置

```typescript
// src/gateway/server-methods.ts

// 方法权限映射
export const METHOD_AUTH: Record<string, {
  requireUser?: boolean
  requireDevice?: boolean
  scopes?: string[]
  public?: boolean
}> = {
  // ========== 公开方法（无需认证）==========
  'auth.sendCode': { public: true },
  'auth.register.phone': { public: true },
  'auth.login.phone': { public: true },
  'auth.login.email': { public: true },
  'auth.refresh': { public: true },
  'subscription.plans.list': { public: true },

  // ========== 需要用户认证 ==========
  'auth.me': { requireUser: true },
  'auth.logout': { requireUser: true },
  'auth.updateProfile': { requireUser: true },
  'auth.changePassword': { requireUser: true },
  'auth.setupMfa': { requireUser: true },
  'device.list': { requireUser: true },
  'device.link': { requireUser: true },
  'subscription.status': { requireUser: true },
  'subscription.create': { requireUser: true },
  'subscription.cancel': { requireUser: true },
  'audit.query': { requireUser: true },

  // ========== 需要设备认证 ==========
  'assistant.fileList': { requireDevice: true, scopes: ['operator.read'] },
  'assistant.fileRead': { requireDevice: true, scopes: ['operator.read'] },
  'assistant.fileWrite': { requireDevice: true, scopes: ['operator.write'] },
  'assistant.skillExecute': { requireDevice: true, scopes: ['operator.write'] },
  'assistant.systemInfo': { requireDevice: true, scopes: ['operator.read'] },

  // ========== 需要用户 + 设备认证 ==========
  'assistant.sensitiveOperation': { requireUser: true, requireDevice: true, scopes: ['operator.admin'] },

  // ========== 管理员权限 ==========
  'device.unlink': { requireUser: true, scopes: ['operator.admin'] },
  'audit.export': { requireUser: true, scopes: ['operator.admin'] },
  'admin.users.list': { requireUser: true, scopes: ['admin'] },
  'admin.subscriptions.list': { requireUser: true, scopes: ['admin'] },
}
```

---

## 7. Drizzle ORM Schema（修订版）

```typescript
// src/db/schema/index.ts

import { pgTable, uuid, varchar, text, boolean, timestamp,
         decimal, integer, jsonb, primaryKey } from 'drizzle-orm/pg-core'
import { relations } from 'drizzle-orm'

// ===== 用户表 =====
export const users = pgTable('users', {
  id: uuid('id').primaryKey().defaultRandom(),

  // 认证信息
  phone: varchar('phone', { length: 20 }).unique(),
  email: varchar('email', { length: 255 }).unique(),
  password_hash: varchar('password_hash', { length: 255 }),
  wechat_openid: varchar('wechat_openid', { length: 100 }).unique(),
  wechat_unionid: varchar('wechat_unionid', { length: 100 }),

  // 用户资料
  display_name: varchar('display_name', { length: 100 }).notNull(),
  avatar_url: varchar('avatar_url', { length: 500 }),

  // 验证状态
  is_phone_verified: boolean('is_phone_verified').default(false),
  is_email_verified: boolean('is_email_verified').default(false),

  // 国际化
  timezone: varchar('timezone', { length: 50 }).default('Asia/Shanghai'),
  locale: varchar('locale', { length: 10 }).default('zh-CN'),

  // 账户状态
  status: varchar('status', { length: 20 }).notNull().default('active'),
  account_type: varchar('account_type', { length: 20 }).default('personal'),

  // MFA
  mfa_enabled: boolean('mfa_enabled').default(false),
  mfa_method: varchar('mfa_method', { length: 20 }),
  mfa_secret: varchar('mfa_secret', { length: 255 }),
  mfa_backup_codes: text('mfa_backup_codes').array(),

  // 安全
  login_attempts: integer('login_attempts').default(0),
  locked_until: timestamp('locked_until', { withTimezone: true }),
  password_changed_at: timestamp('password_changed_at', { withTimezone: true }),

  // 时间戳
  created_at: timestamp('created_at', { withTimezone: true }).defaultNow(),
  updated_at: timestamp('updated_at', { withTimezone: true }).defaultNow(),
  last_login_at: timestamp('last_login_at', { withTimezone: true }),
  deleted_at: timestamp('deleted_at', { withTimezone: true }),
})

// ===== 用户设备关联表 =====
export const userDevices = pgTable('user_devices', {
  id: uuid('id').primaryKey().defaultRandom(),
  user_id: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  device_id: varchar('device_id', { length: 100 }).notNull(),
  display_name: varchar('display_name', { length: 100 }),
  is_primary: boolean('is_primary').default(false),
  linked_at: timestamp('linked_at', { withTimezone: true }).defaultNow(),
  last_used_at: timestamp('last_used_at', { withTimezone: true }),
})

// ===== 用户会话表 =====
export const userSessions = pgTable('user_sessions', {
  id: uuid('id').primaryKey().defaultRandom(),
  user_id: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  refresh_token_hash: varchar('refresh_token_hash', { length: 64 }).notNull(),
  user_device_id: uuid('user_device_id').references(() => userDevices.id, { onDelete: 'set null' }),
  ip_address: varchar('ip_address', { length: 45 }),
  user_agent: varchar('user_agent', { length: 500 }),
  expires_at: timestamp('expires_at', { withTimezone: true }).notNull(),
  revoked_at: timestamp('revoked_at', { withTimezone: true }),
  last_used_at: timestamp('last_used_at', { withTimezone: true }),
  created_at: timestamp('created_at', { withTimezone: true }).defaultNow(),
})

// ===== 登录尝试表 =====
export const loginAttempts = pgTable('login_attempts', {
  id: uuid('id').primaryKey().defaultRandom(),
  user_id: uuid('user_id').references(() => users.id, { onDelete: 'set null' }),
  identifier: varchar('identifier', { length: 255 }).notNull(),
  ip_address: varchar('ip_address', { length: 45 }).notNull(),
  user_agent: varchar('user_agent', { length: 500 }),
  success: boolean('success').notNull(),
  failure_reason: varchar('failure_reason', { length: 50 }),
  attempted_at: timestamp('attempted_at', { withTimezone: true }).defaultNow(),
})

// ===== 验证码表 =====
export const verificationCodes = pgTable('verification_codes', {
  id: uuid('id').primaryKey().defaultRandom(),
  target: varchar('target', { length: 255 }).notNull(),
  target_type: varchar('target_type', { length: 20 }).notNull(),
  code: varchar('code', { length: 10 }).notNull(),
  purpose: varchar('purpose', { length: 20 }).notNull(),
  verified_at: timestamp('verified_at', { withTimezone: true }),
  attempts: integer('attempts').default(0),
  max_attempts: integer('max_attempts').default(5),
  ip_address: varchar('ip_address', { length: 45 }),
  user_agent: varchar('user_agent', { length: 500 }),
  expires_at: timestamp('expires_at', { withTimezone: true }).notNull(),
  created_at: timestamp('created_at', { withTimezone: true }).defaultNow(),
})

// ===== 计划表 =====
export const plans = pgTable('plans', {
  id: uuid('id').primaryKey().defaultRandom(),
  name: varchar('name', { length: 100 }).notNull(),
  slug: varchar('slug', { length: 50 }).notNull().unique(),
  description: text('description'),
  price: decimal('price', { precision: 10, scale: 2 }).notNull(),
  currency: varchar('currency', { length: 3 }).default('CNY'),
  period: varchar('period', { length: 20 }).notNull(),
  features: jsonb('features').notNull().default({}),
  limits: jsonb('limits').notNull().default({}),
  is_active: boolean('is_active').default(true),
  display_order: integer('display_order').default(0),
  created_at: timestamp('created_at', { withTimezone: true }).defaultNow(),
  updated_at: timestamp('updated_at', { withTimezone: true }).defaultNow(),
})

// ===== 技能表 =====
export const skills = pgTable('skills', {
  id: uuid('id').primaryKey().defaultRandom(),
  name: varchar('name', { length: 100 }).notNull(),
  slug: varchar('slug', { length: 50 }).notNull().unique(),
  description: text('description'),
  version: varchar('version', { length: 20 }).notNull(),
  type: varchar('type', { length: 20 }).notNull(),
  category: varchar('category', { length: 50 }),
  tags: text('tags').array().default([]),
  price: decimal('price', { precision: 10, scale: 2 }),
  subscription_only: boolean('subscription_only').default(false),
  code_url: varchar('code_url', { length: 500 }),
  code_hash: varchar('code_hash', { length: 64 }),
  required_permissions: text('required_permissions').array().default([]),
  install_count: integer('install_count').default(0),
  rating: decimal('rating', { precision: 2, scale: 1 }),
  is_active: boolean('is_active').default(true),
  is_featured: boolean('is_featured').default(false),
  created_at: timestamp('created_at', { withTimezone: true }).defaultNow(),
  updated_at: timestamp('updated_at', { withTimezone: true }).defaultNow(),
})

// ===== 用户技能关联表 =====
export const userSkills = pgTable('user_skills', {
  id: uuid('id').primaryKey().defaultRandom(),
  user_id: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  skill_id: uuid('skill_id').notNull().references(() => skills.id, { onDelete: 'cascade' }),
  installed_at: timestamp('installed_at', { withTimezone: true }).defaultNow(),
  configuration: jsonb('configuration').default({}),
  is_favorite: boolean('is_favorite').default(false),
  usage_count: integer('usage_count').default(0),
  last_used_at: timestamp('last_used_at', { withTimezone: true }),
})

// ===== 订阅表 =====
export const subscriptions = pgTable('subscriptions', {
  id: uuid('id').primaryKey().defaultRandom(),
  user_id: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  plan_id: uuid('plan_id').references(() => plans.id),
  skill_id: uuid('skill_id').references(() => skills.id),
  status: varchar('status', { length: 20 }).notNull().default('active'),
  started_at: timestamp('started_at', { withTimezone: true }).defaultNow(),
  expires_at: timestamp('expires_at', { withTimezone: true }),
  cancelled_at: timestamp('cancelled_at', { withTimezone: true }),
  auto_renew: boolean('auto_renew').default(true),
  current_period_end: timestamp('current_period_end', { withTimezone: true }),
  next_billing_at: timestamp('next_billing_at', { withTimezone: true }),
  failed_renew_attempts: integer('failed_renew_attempts').default(0),
  payment_provider: varchar('payment_provider', { length: 50 }),
  payment_id: varchar('payment_id', { length: 100 }),
  amount_paid: decimal('amount_paid', { precision: 10, scale: 2 }),
  created_at: timestamp('created_at', { withTimezone: true }).defaultNow(),
  updated_at: timestamp('updated_at', { withTimezone: true }).defaultNow(),
})

// ===== 支付订单表 =====
export const paymentOrders = pgTable('payment_orders', {
  id: uuid('id').primaryKey().defaultRandom(),
  user_id: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  order_no: varchar('order_no', { length: 50 }).notNull().unique(),
  order_type: varchar('order_type', { length: 20 }).notNull(),
  subscription_id: uuid('subscription_id').references(() => subscriptions.id),
  skill_id: uuid('skill_id').references(() => skills.id),
  plan_id: uuid('plan_id').references(() => plans.id),
  amount: decimal('amount', { precision: 10, scale: 2 }).notNull(),
  currency: varchar('currency', { length: 3 }).default('CNY'),
  discount_amount: decimal('discount_amount', { precision: 10, scale: 2 }).default('0'),
  final_amount: decimal('final_amount', { precision: 10, scale: 2 }).notNull(),
  coupon_id: uuid('coupon_id'),
  coupon_code: varchar('coupon_code', { length: 50 }),
  payment_provider: varchar('payment_provider', { length: 50 }),
  payment_id: varchar('payment_id', { length: 100 }),
  payment_url: varchar('payment_url', { length: 500 }),
  qr_code: text('qr_code'),
  status: varchar('status', { length: 20 }).notNull().default('pending'),
  paid_at: timestamp('paid_at', { withTimezone: true }),
  failed_at: timestamp('failed_at', { withTimezone: true }),
  failure_reason: text('failure_reason'),
  expires_at: timestamp('expires_at', { withTimezone: true }),
  created_at: timestamp('created_at', { withTimezone: true }).defaultNow(),
  updated_at: timestamp('updated_at', { withTimezone: true }).defaultNow(),
})

// ===== 优惠券表 =====
export const couponCodes = pgTable('coupon_codes', {
  id: uuid('id').primaryKey().defaultRandom(),
  code: varchar('code', { length: 50 }).notNull().unique(),
  type: varchar('type', { length: 20 }).notNull(),
  value: decimal('value', { precision: 10, scale: 2 }).notNull(),
  applicable_to: varchar('applicable_to', { length: 20 }).default('all'),
  applicable_ids: uuid('applicable_ids').array(),
  usage_limit: integer('usage_limit'),
  usage_per_user: integer('usage_per_user').default(1),
  minimum_amount: decimal('minimum_amount', { precision: 10, scale: 2 }),
  starts_at: timestamp('starts_at', { withTimezone: true }).notNull(),
  expires_at: timestamp('expires_at', { withTimezone: true }).notNull(),
  is_active: boolean('is_active').default(true),
  created_at: timestamp('created_at', { withTimezone: true }).defaultNow(),
})

// ===== 优惠券使用记录表 =====
export const couponUsage = pgTable('coupon_usage', {
  id: uuid('id').primaryKey().defaultRandom(),
  coupon_id: uuid('coupon_id').notNull().references(() => couponCodes.id),
  user_id: uuid('user_id').notNull().references(() => users.id),
  order_id: uuid('order_id').references(() => paymentOrders.id),
  discount_amount: decimal('discount_amount', { precision: 10, scale: 2 }),
  applied_at: timestamp('applied_at', { withTimezone: true }).defaultNow(),
})

// ===== 审计日志表 =====
export const auditLogs = pgTable('audit_logs', {
  id: uuid('id').defaultRandom(),
  user_id: uuid('user_id').references(() => users.id, { onDelete: 'set null' }),
  device_id: varchar('device_id', { length: 100 }),
  action: varchar('action', { length: 50 }).notNull(),
  resource_type: varchar('resource_type', { length: 50 }).notNull(),
  resource_id: varchar('resource_id', { length: 100 }),
  details: jsonb('details').default({}),
  status: varchar('status', { length: 20 }).notNull(),
  error_message: text('error_message'),
  ip_address: varchar('ip_address', { length: 45 }),
  user_agent: varchar('user_agent', { length: 500 }),
  request_id: varchar('request_id', { length: 50 }),
  risk_level: varchar('risk_level', { length: 20 }).default('low'),
  created_at: timestamp('created_at', { withTimezone: true }).defaultNow(),
}, (table) => ({
  pk: primaryKey({ columns: [table.id, table.created_at] }),
}))

// ===== 导出记录表 =====
export const exportLogs = pgTable('export_logs', {
  id: uuid('id').primaryKey().defaultRandom(),
  user_id: uuid('user_id').notNull().references(() => users.id),
  export_type: varchar('export_type', { length: 50 }).notNull(),
  row_count: integer('row_count'),
  export_size_bytes: integer('export_size_bytes'),
  ip_address: varchar('ip_address', { length: 45 }),
  created_at: timestamp('created_at', { withTimezone: true }).defaultNow(),
})

// ===== 关联关系 =====
export const usersRelations = relations(users, ({ many }) => ({
  devices: many(userDevices),
  sessions: many(userSessions),
  subscriptions: many(subscriptions),
  skills: many(userSkills),
}))

export const userDevicesRelations = relations(userDevices, ({ one }) => ({
  user: one(users, {
    fields: [userDevices.user_id],
    references: [users.id],
  }),
}))

export const subscriptionsRelations = relations(subscriptions, ({ one }) => ({
  user: one(users, {
    fields: [subscriptions.user_id],
    references: [users.id],
  }),
  plan: one(plans, {
    fields: [subscriptions.plan_id],
    references: [plans.id],
  }),
  skill: one(skills, {
    fields: [subscriptions.skill_id],
    references: [skills.id],
  }),
}))

export const userSkillsRelations = relations(userSkills, ({ one }) => ({
  user: one(users, {
    fields: [userSkills.user_id],
    references: [users.id],
  }),
  skill: one(skills, {
    fields: [userSkills.skill_id],
    references: [skills.id],
  }),
}))
```

---

## 8. 数据迁移策略（新增）

### 8.1 迁移概述

由于现有系统使用 JSON 文件存储设备配对信息，需要设计平滑的迁移策略。

```
┌─────────────────────────────────────────────────────────────────────────┐
│                          迁移阶段规划                                    │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│   Phase 1: 准备阶段（无停机）                                            │
│   ├── 部署 PostgreSQL                                                  │
│   ├── 创建数据库表结构                                                  │
│   └── 保留现有 JSON 存储                                               │
│                                                                         │
│   Phase 2: 双写阶段（无停机）                                            │
│   ├── 新设备同时写入 JSON 和 PostgreSQL                                 │
│   ├── 用户注册/登录写入 PostgreSQL                                      │
│   └── 读取仍使用 JSON                                                  │
│                                                                         │
│   Phase 3: 数据同步（后台任务）                                          │
│   ├── 为现有设备创建虚拟用户                                            │
│   ├── 建立 user_devices 关联                                           │
│   └── 验证数据一致性                                                   │
│                                                                         │
│   Phase 4: 切换阶段（计划停机 5 分钟）                                   │
│   ├── 停止 Gateway                                                     │
│   ├── 执行最终数据同步                                                  │
│   ├── 切换读取源为 PostgreSQL                                          │
│   └── 启动 Gateway                                                     │
│                                                                         │
│   Phase 5: 清理阶段（迁移后 30 天）                                      │
│   ├── 移除 JSON 写入逻辑                                               │
│   ├── 归档 JSON 备份                                                   │
│   └── 移除兼容性代码                                                   │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

### 8.2 虚拟用户创建策略

对于没有关联用户的现有设备，创建虚拟用户：

```typescript
// scripts/migrate-devices.ts

/**
 * 为现有配对设备创建虚拟用户
 */
async function migrateExistingDevices(ctx: GatewayContext): Promise<void> {
  const log = ctx.logger.child({ task: 'device-migration' })

  // 1. 读取所有现有配对设备
  const pairedDevices = await devicePairing.getAllDevices()
  log.info(`Found ${pairedDevices.length} paired devices`)

  for (const device of pairedDevices) {
    try {
      // 2. 检查是否已有关联用户
      const existing = await ctx.db.query.userDevices.findFirst({
        where: eq(userDevices.device_id, device.deviceId)
      })

      if (existing) {
        log.debug(`Device ${device.deviceId} already linked`)
        continue
      }

      // 3. 创建虚拟用户
      const virtualEmail = `device-${device.deviceId.slice(0, 8)}@internal.openclaw.local`

      const [user] = await ctx.db.insert(users).values({
        email: virtualEmail,
        display_name: `设备用户 (${device.platform || 'unknown'})`,
        is_email_verified: false,
        status: 'active',
        account_type: 'personal',
      }).returning()

      // 4. 创建设备关联
      await ctx.db.insert(userDevices).values({
        user_id: user.id,
        device_id: device.deviceId,
        display_name: device.displayName || `${device.platform} 设备`,
        is_primary: true,
        linked_at: new Date(device.createdAtMs),
      })

      log.info(`Migrated device ${device.deviceId} to user ${user.id}`)
    } catch (error) {
      log.error(`Failed to migrate device ${device.deviceId}:`, error)
    }
  }
}
```

### 8.3 回滚方案

```typescript
// scripts/rollback-migration.ts

/**
 * 回滚迁移（如果出现问题）
 */
async function rollbackMigration(ctx: GatewayContext): Promise<void> {
  const log = ctx.logger.child({ task: 'migration-rollback' })

  // 1. 切换读取源回 JSON
  ctx.config.deviceStorage = 'json'

  // 2. 禁用 PostgreSQL 写入
  ctx.config.enablePostgresWrite = false

  // 3. 记录回滚事件
  log.warn('Migration rolled back to JSON storage')

  // 4. 发送告警
  await sendAlert({
    level: 'critical',
    message: 'User management migration rolled back',
    details: { reason: 'Manual rollback triggered' }
  })
}
```

---

## 9. 安全加固方案（新增）

### 9.1 密码安全

```typescript
// src/assistant/auth/password.ts

import bcrypt from 'bcrypt'
import zxcvbn from 'zxcvbn'

const BCRYPT_COST = 12

/**
 * 密码强度要求
 */
interface PasswordRequirements {
  minLength: number
  requireUppercase: boolean
  requireLowercase: boolean
  requireNumber: boolean
  requireSpecial: boolean
  minStrength: number  // zxcvbn score (0-4)
}

const PASSWORD_REQUIREMENTS: PasswordRequirements = {
  minLength: 8,
  requireUppercase: true,
  requireLowercase: true,
  requireNumber: true,
  requireSpecial: true,
  minStrength: 2,  // 至少 "fair" 强度
}

/**
 * 验证密码强度
 */
export function validatePasswordStrength(password: string): {
  valid: boolean
  errors: string[]
  score: number
} {
  const errors: string[] = []

  if (password.length < PASSWORD_REQUIREMENTS.minLength) {
    errors.push(`密码至少 ${PASSWORD_REQUIREMENTS.minLength} 个字符`)
  }
  if (PASSWORD_REQUIREMENTS.requireUppercase && !/[A-Z]/.test(password)) {
    errors.push('密码需要包含大写字母')
  }
  if (PASSWORD_REQUIREMENTS.requireLowercase && !/[a-z]/.test(password)) {
    errors.push('密码需要包含小写字母')
  }
  if (PASSWORD_REQUIREMENTS.requireNumber && !/[0-9]/.test(password)) {
    errors.push('密码需要包含数字')
  }
  if (PASSWORD_REQUIREMENTS.requireSpecial && !/[!@#$%^&*(),.?":{}|<>]/.test(password)) {
    errors.push('密码需要包含特殊字符')
  }

  // zxcvbn 强度检查
  const strength = zxcvbn(password)
  if (strength.score < PASSWORD_REQUIREMENTS.minStrength) {
    errors.push('密码强度不足，请使用更复杂的密码')
  }

  return {
    valid: errors.length === 0,
    errors,
    score: strength.score,
  }
}

/**
 * 哈希密码
 */
export async function hashPassword(password: string): Promise<string> {
  return bcrypt.hash(password, BCRYPT_COST)
}

/**
 * 验证密码
 */
export async function verifyPassword(password: string, hash: string): Promise<boolean> {
  return bcrypt.compare(password, hash)
}
```

### 9.2 MFA 实现

```typescript
// src/assistant/auth/mfa.ts

import speakeasy from 'speakeasy'
import qrcode from 'qrcode'
import crypto from 'crypto'

/**
 * 设置 TOTP
 */
export async function setupTOTP(userId: string, displayName: string): Promise<{
  secret: string
  qrCodeUrl: string
  backupCodes: string[]
}> {
  // 1. 生成密钥
  const secret = speakeasy.generateSecret({
    name: `OpenClaw (${displayName})`,
    issuer: 'OpenClaw',
    length: 32,
  })

  // 2. 生成 QR 码
  const qrCodeUrl = await qrcode.toDataURL(secret.otpauth_url!)

  // 3. 生成备用码
  const backupCodes = generateBackupCodes(8)

  return {
    secret: secret.base32,
    qrCodeUrl,
    backupCodes,
  }
}

/**
 * 验证 TOTP
 */
export function verifyTOTP(secret: string, token: string): boolean {
  return speakeasy.totp.verify({
    secret,
    encoding: 'base32',
    token,
    window: 2,  // 允许 ±1 个时间窗口（30秒）
  })
}

/**
 * 生成备用码
 */
function generateBackupCodes(count: number): string[] {
  const codes: string[] = []
  for (let i = 0; i < count; i++) {
    const code = crypto.randomBytes(4).toString('hex').toUpperCase()
    codes.push(`${code.slice(0, 4)}-${code.slice(4)}`)
  }
  return codes
}

/**
 * 验证备用码
 */
export async function verifyBackupCode(
  userId: string,
  code: string,
  ctx: GatewayContext
): Promise<boolean> {
  const user = await ctx.db.query.users.findFirst({
    where: eq(users.id, userId)
  })

  if (!user?.mfa_backup_codes) return false

  const normalizedCode = code.replace('-', '').toUpperCase()
  const codeIndex = user.mfa_backup_codes.findIndex(
    c => c.replace('-', '').toUpperCase() === normalizedCode
  )

  if (codeIndex === -1) return false

  // 使用后移除备用码
  const updatedCodes = [...user.mfa_backup_codes]
  updatedCodes.splice(codeIndex, 1)

  await ctx.db.update(users)
    .set({ mfa_backup_codes: updatedCodes })
    .where(eq(users.id, userId))

  return true
}
```

### 9.3 速率限制

```typescript
// src/gateway/middleware/rate-limit.ts

interface RateLimitConfig {
  windowMs: number      // 时间窗口（毫秒）
  maxRequests: number   // 最大请求数
  keyGenerator: (ctx: GatewayContext) => string
}

const RATE_LIMITS: Record<string, RateLimitConfig> = {
  // 验证码请求：每个 IP 每小时 10 次
  'auth.sendCode': {
    windowMs: 3600000,
    maxRequests: 10,
    keyGenerator: (ctx) => `rate:sendCode:ip:${ctx.req?.ip}`,
  },

  // 登录尝试：每个 IP 每 15 分钟 20 次
  'auth.login': {
    windowMs: 900000,
    maxRequests: 20,
    keyGenerator: (ctx) => `rate:login:ip:${ctx.req?.ip}`,
  },

  // 审计日志导出：每个用户每小时 3 次
  'audit.export': {
    windowMs: 3600000,
    maxRequests: 3,
    keyGenerator: (ctx) => `rate:export:user:${ctx.auth?.user?.id}`,
  },

  // 技能执行：每个设备每分钟 60 次
  'assistant.skillExecute': {
    windowMs: 60000,
    maxRequests: 60,
    keyGenerator: (ctx) => `rate:skill:device:${ctx.auth?.device?.deviceId}`,
  },
}

/**
 * 检查速率限制
 */
export async function checkRateLimit(
  method: string,
  ctx: GatewayContext
): Promise<{ allowed: boolean; remaining: number; resetAt: Date }> {
  const config = RATE_LIMITS[method]
  if (!config) {
    return { allowed: true, remaining: -1, resetAt: new Date() }
  }

  const key = config.keyGenerator(ctx)
  const now = Date.now()
  const windowStart = now - config.windowMs

  // 使用 Redis 或内存缓存实现
  // 这里简化使用内存实现
  const count = await incrementRateCounter(key, windowStart)

  const allowed = count <= config.maxRequests
  const remaining = Math.max(0, config.maxRequests - count)
  const resetAt = new Date(now + config.windowMs)

  if (!allowed) {
    await logAudit(ctx, {
      user_id: ctx.auth?.user?.id,
      device_id: ctx.auth?.device?.deviceId,
      action: 'rate_limit.exceeded',
      resource_type: 'api',
      resource_id: method,
      status: 'denied',
      details: { count, limit: config.maxRequests },
      risk_level: 'medium',
    })
  }

  return { allowed, remaining, resetAt }
}
```

---

## 10. 配置与环境变量

### 10.1 环境变量配置

```bash
# .env.example

# ========== 数据库 ==========
DATABASE_URL=postgresql://user:password@localhost:5432/openclaw
DATABASE_POOL_MIN=2
DATABASE_POOL_MAX=10
DATABASE_SSL=false

# ========== JWT ==========
JWT_SECRET=your-256-bit-secret-key-here
JWT_ACCESS_TOKEN_EXPIRY=15m
JWT_REFRESH_TOKEN_EXPIRY=7d

# ========== 短信服务 ==========
SMS_PROVIDER=aliyun
SMS_ACCESS_KEY_ID=your-access-key
SMS_ACCESS_KEY_SECRET=your-access-secret
SMS_SIGN_NAME=OpenClaw
SMS_TEMPLATE_CODE=SMS_123456789

# ========== 邮件服务 ==========
EMAIL_PROVIDER=smtp
SMTP_HOST=smtp.example.com
SMTP_PORT=587
SMTP_USER=noreply@openclaw.ai
SMTP_PASSWORD=your-smtp-password
EMAIL_FROM=noreply@openclaw.ai

# ========== 微信登录 ==========
WECHAT_APP_ID=wx123456789
WECHAT_APP_SECRET=your-wechat-secret
WECHAT_CALLBACK_URL=https://api.openclaw.ai/auth/wechat/callback

# ========== 支付网关 ==========
# 微信支付
WECHAT_PAY_MCH_ID=your-mch-id
WECHAT_PAY_KEY=your-api-key
WECHAT_PAY_NOTIFY_URL=https://api.openclaw.ai/payment/wechat/notify

# 支付宝
ALIPAY_APP_ID=your-app-id
ALIPAY_MERCHANT_PRIVATE_KEY=your-private-key
ALIPAY_ALIPAY_PUBLIC_KEY=alipay-public-key
ALIPAY_NOTIFY_URL=https://api.openclaw.ai/payment/alipay/notify

# ========== 功能开关 ==========
FEATURE_SMS_REGISTRATION=true
FEATURE_EMAIL_REGISTRATION=true
FEATURE_WECHAT_LOGIN=true
FEATURE_MFA=true
FEATURE_SUBSCRIPTIONS=true
FEATURE_PAYMENTS=true

# ========== 安全 ==========
BCRYPT_COST=12
MAX_LOGIN_ATTEMPTS=5
ACCOUNT_LOCK_DURATION_MINUTES=15
```

### 10.2 配置文件结构

```yaml
# ~/.openclaw/config.yaml

# 数据库配置
database:
  url: "${DATABASE_URL}"
  pool:
    min: 2
    max: 10
  ssl: false

# JWT 配置
jwt:
  secret: "${JWT_SECRET}"
  accessToken:
    expiry: "15m"
  refreshToken:
    expiry: "7d"
    maxSessionsPerUser: 5

# 认证配置
auth:
  sms:
    provider: "${SMS_PROVIDER}"
    codeExpiry: 300  # 5 分钟
    codeLength: 6
    rateLimit:
      perIpPerHour: 10
      perPhonePerMinute: 1
  email:
    provider: "${EMAIL_PROVIDER}"
    from: "${EMAIL_FROM}"
  password:
    minLength: 8
    requireUppercase: true
    requireLowercase: true
    requireNumber: true
    requireSpecial: true
    bcryptCost: 12
  mfa:
    enabled: true
    backupCodesCount: 8
  lockout:
    maxAttempts: 5
    durationMinutes: 15

# 审计配置
audit:
  enabled: true
  retentionDays: 90
  partitionByMonth: true
  alertOnHighRisk: true

# 订阅配置
subscription:
  autoRenewal:
    enabled: true
    cronSchedule: "0 2 * * *"  # 每天凌晨 2 点
    maxRetries: 3
  gracePeriodDays: 3
```

---

## 11. 实施计划（修订版）

| 阶段 | 时间 | 任务 | 产出 | 依赖 |
|------|------|------|------|------|
| **Phase 1** | Week 1 | 数据库基础 | PostgreSQL 配置、Schema、迁移 | - |
| **Phase 2** | Week 2 | 用户认证 | 注册/登录、JWT、MFA | Phase 1 |
| **Phase 3** | Week 3 | 设备整合 | 用户设备关联、现有系统兼容 | Phase 2 |
| **Phase 4** | Week 4 | 订阅系统 | 计划管理、支付集成、自动续费 | Phase 2 |
| **Phase 5** | Week 5 | 审计日志 | 日志记录、查询、导出、告警 | Phase 2 |
| **Phase 6** | Week 6 | 数据迁移 | 现有设备迁移、虚拟用户创建 | Phase 3 |
| **Phase 7** | Week 7 | 安全加固 | 速率限制、安全审计、监控 | All |
| **Phase 8** | Week 8 | 整合测试 | E2E 测试、性能测试、文档 | All |

### 11.1 Phase 1 详细任务

- [x] 设计数据库 Schema
- [ ] 安装和配置 PostgreSQL 16
- [ ] 配置 Drizzle ORM
- [ ] 创建所有数据库表
- [ ] 编写迁移脚本
- [ ] 配置连接池
- [ ] 编写数据库工具函数

### 11.2 Phase 2 详细任务

- [ ] 实现密码强度验证
- [ ] 实现验证码发送服务（防暴力破解）
- [ ] 实现手机号注册/登录
- [ ] 实现邮箱密码注册/登录
- [ ] 实现 JWT Token 生成/验证
- [ ] 实现 Token 刷新机制
- [ ] 实现 MFA (TOTP + 备用码)
- [ ] 实现账户锁定机制

### 11.3 Phase 3 详细任务

- [ ] 实现 user_devices 关联表逻辑
- [ ] 与现有 device-pairing.ts 整合
- [ ] 实现设备绑定 API
- [ ] 实现设备解绑 API
- [ ] 实现设备配额检查

### 11.4 Phase 4 详细任务

- [ ] 实现计划管理 API
- [ ] 实现支付订单创建
- [ ] 集成微信支付
- [ ] 集成支付宝支付
- [ ] 实现支付回调处理
- [ ] 实现优惠券系统
- [ ] 实现自动续费定时任务
- [ ] 实现使用限额检查

### 11.5 Phase 5 详细任务

- [ ] 实现审计日志记录服务
- [ ] 实现风险级别自动判定
- [ ] 实现审计日志查询 API（分页）
- [ ] 实现日志导出功能（频率限制）
- [ ] 实现高风险操作告警
- [ ] 配置日志分区
- [ ] 实现日志清理定时任务

### 11.6 Phase 6 详细任务

- [ ] 编写设备迁移脚本
- [ ] 实现虚拟用户创建逻辑
- [ ] 实现双写模式
- [ ] 执行数据同步
- [ ] 验证数据一致性
- [ ] 编写回滚脚本

### 11.7 Phase 7 详细任务

- [ ] 实现 API 速率限制
- [ ] 实现安全告警系统
- [ ] 配置监控和报警
- [ ] 安全审计检查
- [ ] 渗透测试（可选）

### 11.8 Phase 8 详细任务

- [ ] 编写单元测试
- [ ] 编写集成测试
- [ ] 编写 E2E 测试
- [ ] 性能测试
- [ ] 编写 API 文档
- [ ] 编写运维文档

---

## 12. 项目结构（修订版）

```
src/
├── db/                            # 🆕 数据库层
│   ├── index.ts                   # 数据库连接和初始化
│   ├── schema/                    # Drizzle Schema
│   │   ├── index.ts               # 导出所有 schema
│   │   ├── users.ts               # 用户相关表
│   │   ├── subscriptions.ts       # 订阅相关表
│   │   └── audit.ts               # 审计相关表
│   └── migrations/                # 迁移文件
│       ├── 0001_create_users.sql
│       ├── 0002_create_subscriptions.sql
│       └── 0003_create_audit_logs.sql
│
├── assistant/                     # 🆕 助理系统
│   ├── auth/                      # 认证模块
│   │   ├── jwt.ts                 # JWT 处理
│   │   ├── password.ts            # 密码哈希和验证
│   │   ├── mfa.ts                 # MFA 实现
│   │   └── verification.ts        # 验证码服务
│   ├── subscription/              # 订阅模块
│   │   ├── service.ts             # 订阅服务
│   │   ├── coupon.ts              # 优惠券逻辑
│   │   └── auto-renew.ts          # 自动续费
│   ├── audit/                     # 审计模块
│   │   └── service.ts             # 审计日志服务
│   └── payment/                   # 支付模块
│       ├── wechat.ts              # 微信支付
│       └── alipay.ts              # 支付宝支付
│
├── gateway/
│   ├── middleware/                # 🆕 中间件
│   │   ├── auth.ts                # 统一认证
│   │   └── rate-limit.ts          # 速率限制
│   └── server-methods/
│       ├── auth.ts                # 🆕 认证方法
│       ├── assistant-subscription.ts
│       ├── assistant-audit.ts
│       ├── assistant-payment.ts
│       └── assistant.ts
│
├── infra/
│   └── device-pairing.ts          # 现有（保持兼容）
│
└── scripts/
    ├── migrate-devices.ts         # 🆕 设备迁移脚本
    └── rollback-migration.ts      # 🆕 回滚脚本
```

---

*— 文档修订完成 —*

...
read_when:
  - "You installed MtBot but `mtbot` is “command not found”"
...
MtBot’s runtime baseline is **Node 22+**.

If you can run `npm install -g mtbot@latest` but later see `mtbot: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.

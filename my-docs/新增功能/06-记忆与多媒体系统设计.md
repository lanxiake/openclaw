...
export class MemoryManager extends EventEmitter {
  ...
  async search(
    ...
    const [facts, episodes, knowledge] = await Promise.all([
...
export class Mem0ProfileMemoryProvider implements IProfileMemoryProvider {
  ...
  async addFact(userId: string, fact: Omit<UserFact, "id" | "createdAt">): Promise<string> {
...
export class GraphitiKnowledgeProvider implements IKnowledgeMemoryProvider {
  ...
  private async extractToGraph(
    ...
    for (const chunk of chunks) {
      // 使用 Graphiti 添加到图谱
      await this.graphiti.addEpisode(chunk.text, {
        userId,
        documentId,
        source: "document",
        timestamp: new Date(),
...
memory:
  ...
  storage:
    ...
    options:
      ...
      # Bucket 配置
      buckets:
        documents: mtbot-documents
        media: mtbot-media
        embeddings: mtbot-embeddings
        exports: mtbot-exports

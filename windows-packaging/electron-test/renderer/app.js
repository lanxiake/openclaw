...
function sendConnect() {
  ...
  const auth = (state.gatewayToken || state.password)
...
async function sendChatMessage() {
  ...
  try {
    await sendRpc('chat.send', {
      ...
      message: text,
      idempotencyKey,
      attachments: userMessage.attachments.length > 0 ? userMessage.attachments.map(a => ({
        type: 'image',
        mimeType: a.mimeType,
...
function renderAttachments() {
  ...
  container.innerHTML = state.attachments.map((att, index) => `
...
function updateOverviewUI() {
  ...
  if (statCron) statCron.textContent = state.cronEnabled == null ? '--' : (state.cronEnabled ? '启用' : '禁用');
  if (statCronNext) statCronNext.textContent = state.cronNext ? `下次运行: ${formatAgo(state.cronNext)}` : '下次运行: --';
...
async function toggleSkill(skillKey, currentlyDisabled) {
  ...
  try {
    ...
    state.skillMessages[skillKey] = { kind: 'success', message: currentlyDisabled ? '已启用' : '已禁用' };
    await loadSkills();
  } catch (err) {
...
function renderSkillsList() {
  ...
  container.innerHTML = filtered.map(skill => {
    ...
    const sourceClass = skill.source === 'bundled' ? 'bundled' :
                        skill.source === 'managed' ? 'managed' : 'workspace';

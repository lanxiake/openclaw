...
IMAGE_NAME="mtbot-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="mtbot-net-e2e-$$"
GW_NAME="mtbot-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "MTBOT_GATEWAY_TOKEN=$TOKEN" \
	  -e "MTBOT_SKIP_CHANNELS=1" \
	  -e "MTBOT_SKIP_GMAIL_WATCHER=1" \
	  -e "MTBOT_SKIP_CRON=1" \
	  -e "MTBOT_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \

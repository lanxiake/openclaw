...
You can also set `MTBOT_VERBOSE=1` instead of the flag.
...
### `docs.mtbot.top` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.mtbot.top` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.mtbot.top` to the allowlist, then retry.
...
- Check the allowlist: `mtbot config get agents.defaults.models`
...
If you can, include the relevant log tail from `mtbot logs --follow`.

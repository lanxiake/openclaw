...
**最后更新**: 2026-02-11 09:30

| 指标            | 数值                    |
| --------------- | ----------------------- |
| 已完成测试文件  | 10 + 33(unit) + 21(e2e) + 4(browser) = 68 |
| 已通过测试用例  | 214 + 209 + 103 + 34 = 560 |
| 失败测试用例    | 9 (Gateway E2E) + 1 (前端 AC-AUTH-005) = 10 |
| 当前代码覆盖率  | ~25% (后端服务 + Gateway) |
| P0 优先级完成度 | 85.7% (6/7)             |
| 前端浏览器测试  | admin-console 17/18 ✅ web-admin 17/17 ✅ |
| 桌面测试        | 待执行                  |
| Schema 问题     | 3 个已修复              |
...
**浏览器测试**: ✅ admin-console 认证 (4/5) ✅ admin-console 用户管理 (5/5) ✅ admin-console 订阅+Gateway (8/8) ✅ web-admin 全量 (17/17)
...
**前端修复**: ✅ web-admin authStore 缺少 hasScope 方法 ✅ web-admin 订阅页面 features 对象渲染错误
...
| AC-AUTH-005  | Token 过期自动跳转 | Token 过期后操作 | 跳转登录页并提示 | P1 | ❌ 前端未校验 Token 有效性 |
...
| -------------- | ------------ | -- | -- | -- | ------ | ---- |
| admin-console  | 37           | 14 | 14 | 9  | 17     | 🟡 P0 基本通过 |
| web-admin      | 22           | 11 | 8  | 3  | 17     | ✅ P0 全部通过 |
...
| 14 | admin-console | AC-AUTH-005: 前端仅检查 Token 是否存在，不校验 Token 有效性/过期时间 | ❌ 功能缺失 (P1) |
...
**阶段 B — 前后端联调测试**
- [x] admin-console P0 用例通过（AC-AUTH + AC-USER + AC-SUB + AC-GW）— 17/18 通过，仅 AC-AUTH-005 (P1) 失败

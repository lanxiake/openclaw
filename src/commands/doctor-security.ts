...
import type { MtBotConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: MtBotConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("mtbot security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("mtbot configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("mtbot config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("mtbot doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("mtbot config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {

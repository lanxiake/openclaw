...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.mtbot/mtbot.json. Prefer MTBOT_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.mtbot/mtbot.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.mtbot/mtbot.json. Restart the gateway to apply."
        }

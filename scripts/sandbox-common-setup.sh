...
BASE_IMAGE="${BASE_IMAGE:-mtbot-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-mtbot-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=mtbot.sandbox=1)
NOTE

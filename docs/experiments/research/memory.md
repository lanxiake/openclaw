...
read_when:
  - Designing workspace memory (~/.mtbot/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep MtBot integration
  - Adding offline recall + reflection (retain/recall/reflect)
---

# Workspace Memory v2 (offline): research notes

Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.mtbot/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.mtbot/workspace` as canonical human-readable memory.
...
```
~/.mtbot/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in MtBot**, but keep a separable core library.

### Why integrate into MtBot?
...
- You want the agent itself to call the tools:
  - `mtbot memory recall "…" --k 25 --since 30d`
  - `mtbot memory reflect --since 7d`
...
Pragmatic take for `~/.mtbot/workspace`:

...
# Build and bundle MtBot into a minimal .app we can open.
# Outputs to dist/MtBot.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/MtBot.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="MtBot"
BUNDLE_ID="${BUNDLE_ID:-com.yunkun.mtbot.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/mtbot/mtbot/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/MtBot/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :MtBotBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :MtBotGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/MtBot"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/MtBot"
fi
chmod +x "$APP_ROOT/Contents/MacOS/MtBot"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/MtBot" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/MtBot/Resources/MtBot.icns" "$APP_ROOT/Contents/Resources/MtBot.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/MtBot/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying MtBotKit resources"
MTBOTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/MtBotKit_MtBotKit.bundle"
if [ -d "$MTBOTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/MtBotKit_MtBotKit.bundle"
  cp -R "$MTBOTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/MtBotKit_MtBotKit.bundle"
else
  echo "WARN: MtBotKit resource bundle not found at $MTBOTKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running MtBot"
killall -q MtBot 2>/dev/null || true

...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    skipBrowser: process.env.MTBOT_SKIP_BROWSER_CONTROL_SERVER,
    token: process.env.MTBOT_GATEWAY_TOKEN,
    password: process.env.MTBOT_GATEWAY_PASSWORD,
  };
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "mtbot-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.MTBOT_STATE_DIR = prev.stateDir;
    ...
    process.env.MTBOT_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
    process.env.MTBOT_GATEWAY_TOKEN = prev.token;
    process.env.MTBOT_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "mtbot");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.MTBOT_STATE_DIR = stateDir;
    process.env.MTBOT_CONFIG_PATH = path.join(stateDir, "mtbot.json");

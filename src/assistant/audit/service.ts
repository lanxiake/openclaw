...
// 审计日志目录
const AUDIT_LOG_DIR = process.env.OPENCLAW_AUDIT_DIR || path.join(
...
async function cleanupOldLogs(): Promise<void> {
  ...
  try {
    ...
    if (deletedCount > 0) {
      logger.info(`[${LOG_TAG}] 清理过期日志`, { deletedCount, retentionDays: currentConfig.retentionDays })
    }
  } catch (err) {
...
export async function writeAuditLog(input: CreateAuditLogInput): Promise<AuditLogEntry> {
  ...
  if (severityOrder[severity] < severityOrder[currentConfig.minSeverity]) {
    logger.debug(`[${LOG_TAG}] 严重级别被过滤`, { severity, minSeverity: currentConfig.minSeverity })
    return createEntry(input)
  }
  ...
  // 处理隐私设置
  if (!currentConfig.logChatContent && (input.eventType === 'chat.message.sent' || input.eventType === 'chat.message.received')) {
    // 隐藏聊天内容
...
export async function queryAuditLogs(filters: AuditLogFilters = {}): Promise<AuditLogQueryResult> {
  ...
  logger.debug(`[${LOG_TAG}] 查询结果`, { total, offset, limit, returned: paginatedEntries.length })
...
export async function getAuditStats(): Promise<AuditLogStats> {
  ...
  logger.debug(`[${LOG_TAG}] 统计结果`, { totalEntries: stats.totalEntries, todayCount, weekCount })
...
export async function exportAuditLogs(options: AuditExportOptions): Promise<string> {
  ...
  const fields = options.fields || [
    'id', 'timestamp', 'eventType', 'severity', 'title', 'detail', 'result'
  ] as Array<keyof AuditLogEntry>
 ...
 * 清除审计日志
 */
export async function clearAuditLogs(options?: { beforeDate?: string }): Promise<{ deletedCount: number }> {
  // 确保已初始化
  if (!initialized) {
    await initAuditLog()
  }

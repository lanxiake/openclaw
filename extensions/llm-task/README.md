...
`mtbot.invoke --each`) without adding new MtBot code per workflow.
...
This extension depends on MtBot internal modules (the embedded agent runner).
It is intended to ship as a **bundled** MtBot extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.mtbot/extensions` as a standalone plugin directory.

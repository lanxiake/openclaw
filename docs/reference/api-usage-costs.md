...
MtBot features that can generate provider usage or paid API calls.
...
- `mtbot status --usage` and `mtbot channels list` show provider **usage windows**
...
MtBot can pick up credentials from:
...
`mtbot models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.

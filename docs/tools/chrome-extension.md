---
summary: "Chrome extension: let MtBot drive your existing Chrome tab"
read_when:
...
The MtBot Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate mtbot-managed Chrome profile.
...
MtBot then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the MtBot release (npm package) as static files. There is no separate “build” step.

After upgrading MtBot:

- Re-run `mtbot browser extension install` to refresh the installed files under your MtBot state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

MtBot ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want MtBot to control.
...
- `ON`: attached; MtBot can drive that tab.
...
Debugging: `mtbot sandbox explain`
...
`mtbot browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `mtbot browser extension install` first to copy the extension to a stable location under your MtBot state directory.
...
- **This is not isolated** like the dedicated mtbot-managed profile.

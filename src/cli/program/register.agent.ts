...
${formatHelpExamples([
  ['mtbot agent --to +15555550123 --message "status update"', "Start a new session."],
  ['mtbot agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'mtbot agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'mtbot agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['mtbot agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'mtbot agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.mtbot.top/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.mtbot.top/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['mtbot agents set-identity --agent main --name "MtBot" --emoji "🦞"', "Set name + emoji."],
  ["mtbot agents set-identity --agent main --avatar avatars/mtbot.png", "Set avatar path."],
  [
    "mtbot agents set-identity --workspace ~/.mtbot/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "mtbot agents set-identity --identity-file ~/.mtbot/workspace/IDENTITY.md --agent main",

...
Display HTML content on connected MtBot nodes (Mac app, iOS, Android).
...
```bash
cat ~/.mtbot/mtbot.json | jq '.gateway.bind'
```
...
1. Check server bind: `cat ~/.mtbot/mtbot.json | jq '.gateway.bind'`
...
Node is offline. Use `mtbot nodes list` to find online nodes.

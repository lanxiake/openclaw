...
INSTALL_URL="${MTBOT_INSTALL_URL:-https://mtbot.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${MTBOT_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${MTBOT_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="mtbot"
PACKAGE_NAME="${MTBOT_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (MTBOT_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${MTBOT_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${MTBOT_INSTALL_LATEST_OUT:-}"
fi

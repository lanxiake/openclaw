export const MAX_PAYLOAD_BYTES = 10 * 1024 * 1024; // 10MB — cap incoming frame size (supports image attachments)
...
export const getHandshakeTimeoutMs = () => {
  if (process.env.VITEST && process.env.MTBOT_TEST_HANDSHAKE_TIMEOUT_MS) {
    const parsed = Number(process.env.MTBOT_TEST_HANDSHAKE_TIMEOUT_MS);

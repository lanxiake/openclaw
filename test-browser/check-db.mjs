...
try {
  ...
  // 查看当前失败记录
  const attempts =
    await sql`SELECT count(*)::int as total FROM admin_login_attempts WHERE success = false`;
  ...
  const admins =
    await sql`SELECT username, status, failed_login_attempts, locked_until FROM admins WHERE username = 'admin'`;

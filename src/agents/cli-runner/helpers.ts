...
/**
 * Cleanup suspended MtBot CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: MtBotConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mtbot-cli-images-"));
  const paths: string[] = [];

...
homepage: https://docs.mtbot.top/hooks#command-logger
metadata:
  {
    "mtbot":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.mtbot/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.mtbot/logs/commands.log ~/.mtbot/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/mtbot`:
   ```
   /home/username/.mtbot/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok

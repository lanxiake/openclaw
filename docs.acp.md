...
This document describes how the MtBot ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`mtbot acp` exposes an ACP agent over stdio and forwards prompts to a running
MtBot Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a MtBot Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `mtbot acp` over stdio.

Example config:

```bash
mtbot config set gateway.remote.url wss://gateway-host:18789
mtbot config set gateway.remote.token <token>
```

Example run:

```bash
mtbot acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
mtbot acp --session agent:main:main
mtbot acp --session agent:design:main
mtbot acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “MtBot ACP” to start a thread.

## Execution Model

- ACP client spawns `mtbot acp` and speaks ACP messages over stdio.
...
```bash
mtbot acp --session agent:main:main
mtbot acp --session-label "support inbox"
mtbot acp --reset-session
```
...
`mtbot acp` resolves the Gateway URL and auth from CLI flags or config:

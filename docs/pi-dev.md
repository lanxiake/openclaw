...
This guide summarizes a sane workflow for working on the pi integration in MtBot.
...
- Trigger the agent directly:
  - `pnpm mtbot agent --message "Hello" --thinking low`
...
State lives under the MtBot state directory. Default is `~/.mtbot`. If `MTBOT_STATE_DIR` is set, use that directory instead.

To reset everything:

- `mtbot.json` for config
...
- https://docs.mtbot.top/testing
- https://docs.mtbot.top/start/getting-started

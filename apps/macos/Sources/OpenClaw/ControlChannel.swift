...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: MtBotProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "com.yunkun.mtbot", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: MtBotKit.AnyCodable]()) {
                $0[$1.key] = MtBotKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: MtBotProtocol.AnyCodable?) -> [String: MtBotProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: MtBotProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: MtBotKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: MtBotProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("mtbot.control.heartbeat")
    static let controlAgentEvent = Notification.Name("mtbot.control.agent")
}

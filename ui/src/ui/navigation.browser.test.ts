...
import { MtBotApp } from "./app";
import "../styles.css";

const originalConnect = MtBotApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("mtbot-app") as MtBotApp;
...
beforeEach(() => {
  MtBotApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__MTBOT_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  MtBotApp.prototype.connect = originalConnect;
  window.__MTBOT_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/mtbot/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/mtbot");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/mtbot/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__MTBOT_CONTROL_UI_BASE_PATH__ = "/mtbot";
    const app = mountApp("/mtbot/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/mtbot");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/mtbot/sessions");
  });

  ...
  function renderListenList() {
      ...
      if (chats.length === 0) {
          container.innerHTML = '<div class="empty-state"><span class="empty-icon">📭</span><p>暂无监听项</p><p
  class="empty-hint">点击"从微信获取"或手动添加</p></div>';
          return;
      }
      ...
      chats.forEach(function(chat) {
          ...
          item.innerHTML = '<input type="checkbox" ' + checkedAttr + ' onchange="toggleListenChat(\'' + escapeHtml(chat.name) + '\',      
  this.checked)">' +
              '<div class="listen-item-info"><span class="listen-item-name">' + escapeHtml(chat.name) + '</span>' +
              '<span class="listen-item-type ' + chat.type + '">' + typeLabel + '</span></div>' +
  ...
  function showToast(message, type) {
      ...
      toast.innerHTML = '<span class="toast-icon">' + icons[type] + '</span><span class="toast-message">' + escapeHtml(message) +
  '</span>';

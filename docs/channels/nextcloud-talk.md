...
```bash
mtbot plugins install @mtbot/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
mtbot plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "MtBot" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `mtbot pairing list nextcloud-talk`
  - `mtbot pairing approve nextcloud-talk <CODE>`

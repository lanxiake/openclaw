...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.MTBOT_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*mtbot-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "mtbot";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "mtbot");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "mtbot-gw");
  ...
  records.push(`_mtbot-gw._tcp IN PTR ${instanceLabel}._mtbot-gw._tcp`);
  records.push(`${instanceLabel}._mtbot-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._mtbot-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; mtbot-content-hash: ${contentHash}\n${contentBody}`;
}

...
enum CommandResolver {
    private static let projectRootDefaultsKey = "mtbot.gatewayProjectRootPath"
    private static let helperName = "mtbot"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let mtbotEntry = root.appendingPathComponent("mtbot.mjs").path
        if FileManager().isReadableFile(atPath: mtbotEntry) { return mtbotEntry }
        let binEntry = root.appendingPathComponent("bin/mtbot.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/mtbot")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let mtbotPaths = self.mtbotManagedPaths(home: home)
        if !mtbotPaths.isEmpty {
            extras.insert(contentsOf: mtbotPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + mtbotPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func mtbotManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".mtbot"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func mtbotExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectMtBotExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("mtbot.mjs").path,
            root.appendingPathComponent("bin/mtbot.js").path,
        ]
    ...
    static func hasAnyMtBotInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.mtbotExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let mtbotPath = self.projectMtBotExecutable(projectRoot: root) {
                return [mtbotPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "mtbot", subcommand] + extraArgs
            }
            if let mtbotPath = self.mtbotExecutable(searchPaths: searchPaths) {
                return [mtbotPath, subcommand] + extraArgs
            }

            let missingEntry = """
            mtbot entrypoint missing (looked for dist/index.js or mtbot.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real mtbot CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/mtbot"
        ...
        \(cliSection)
        \(projectSection)
        if command -v mtbot >/dev/null 2>&1; then
          CLI="$(command -v mtbot)"
          mtbot \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/mtbot.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/mtbot.mjs"
            node "$PRJ/mtbot.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/mtbot.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/mtbot.js"
            node "$PRJ/bin/mtbot.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent mtbot"
          pnpm --silent mtbot \(quotedArgs);
        else
          echo "mtbot CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? MtBotConfigFile.loadDict()

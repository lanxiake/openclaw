#!/bin/bash
# Setup MtBot Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'mtbot-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  MtBot message: Send warning via MtBot itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/mtbot-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/mtbot-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/mtbot-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now mtbot-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status mtbot-auth-monitor.timer"
echo ""
echo "Quick commands:"

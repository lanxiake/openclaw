...
const ETC_MTBOT_DIR = path.join(ROOT_DIR, "etc", "mtbot");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "mtbot.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcMtBotPath(...parts: string[]) {
  return path.join(ETC_MTBOT_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcMtBotPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "mtbot.json"))).toEqual({
      shared: true,
    });
  });
});

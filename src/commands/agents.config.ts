...
export function listAgentEntries(cfg: MtBotConfig): AgentEntry[] {
...
function resolveAgentName(cfg: MtBotConfig, agentId: string) {
...
function resolveAgentModel(cfg: MtBotConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: MtBotConfig): AgentSummary[] {

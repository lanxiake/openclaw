...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.MTBOT_STATE_DIR;
  previousConfigPath = process.env.MTBOT_CONFIG_PATH;
  previousSkipBrowserControl = process.env.MTBOT_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.MTBOT_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.MTBOT_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "mtbot-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.MTBOT_STATE_DIR = path.join(tempHome, ".mtbot");
  delete process.env.MTBOT_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.MTBOT_SKIP_BROWSER_CONTROL_SERVER = "1";
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "mtbot-test-"))
    : path.join(tempHome, ".mtbot-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.MTBOT_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.MTBOT_CONFIG_PATH;
    } else {
      process.env.MTBOT_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.MTBOT_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.MTBOT_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.MTBOT_SKIP_GMAIL_WATCHER;
    } else {
      process.env.MTBOT_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.MTBOT_SKIP_CANVAS_HOST;
    } else {
      process.env.MTBOT_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.MTBOT_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.MTBOT_GATEWAY_TOKEN = fallbackToken;
  }

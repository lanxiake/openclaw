...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mtbot-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "mtbot doctor",
        managerScriptArgs(manager, "mtbot", ["doctor", "--non-interactive"]),
        gitRoot,
        { MTBOT_UPDATE_IN_PROGRESS: "1" },
      ),
    );

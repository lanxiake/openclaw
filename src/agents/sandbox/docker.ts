...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "mtbot.sandbox=1");
  args.push("--label", `mtbot.sessionKey=${params.scopeKey}`);
  args.push("--label", `mtbot.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `mtbot.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("mtbot.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`mtbot sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`mtbot sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("mtbot sandbox recreate --all");
}

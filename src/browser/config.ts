import type { BrowserConfig, BrowserProfileConfig, MtBotConfig } from "../config/config.js";
import {
...
import {
  DEFAULT_MTBOT_BROWSER_COLOR,
  DEFAULT_MTBOT_BROWSER_ENABLED,
  DEFAULT_BROWSER_EVALUATE_ENABLED,
  DEFAULT_BROWSER_DEFAULT_PROFILE_NAME,
  DEFAULT_MTBOT_BROWSER_PROFILE_NAME,
} from "./constants.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_MTBOT_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "mtbot" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_MTBOT_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_MTBOT_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an MtBot browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for mtbot/mtbot CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: MtBotConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_MTBOT_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "mtbot";

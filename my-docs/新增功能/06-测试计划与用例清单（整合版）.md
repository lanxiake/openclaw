...
> 版本: 2.5 | 创建日期: 2026-02-08 | 更新日期: 2026-02-11 | 状态: 执行中
>
...
| 2.5  | 2026-02-11 | 完成阶段B 前端浏览器测试：admin-console 17/18通过 + web-admin 17/17通过，修复2个前端bug |
...
**最后更新**: 2026-02-11 08:35

| 指标            | 数值                    |
| --------------- | ----------------------- |
| 已完成测试文件  | 10 + 33(unit) + 21(e2e) = 64 |
| 已通过测试用例  | 214 + 209 + 103 = 526   |
| 失败测试用例    | 9 (Gateway E2E 环境相关) |
...
| Gateway RPC 方法                    | 集成测试 | P1     | 🟡 进行中 (单元209通过, E2E 103/114) |
| admin-console 功能                  | 前端测试 | P0     | ⏳ 待测试              |
| web-admin 功能                      | 前端测试 | P0     | ⏳ 待测试              |
| windows 桌面功能                    | E2E/手动 | P0     | ⏳ 待测试              |
...
describe("auth-service: register", () => {
  ...
  it("AUTH-REG-001: 应该成功注册手机号用户", async () => {
    const result = await register({
      ...
      password: "Test@123", displayName: "测试用户",
    });
    expect(result.success).toBe(true);
...
| AC-AUTH-005  | Token 过期自动跳转 | Token 过期后操作 | 跳转登录页并提示 | P1 | ⏳ |
...
| AC-USER-003  | 用户详情       | 点击用户行 | 显示用户完整信息 | P0 | ⏳ |
| AC-USER-004  | 编辑用户       | 修改用户状态 → 保存 | 状态更新成功 | P0 | ⏳ |
| AC-USER-005  | 分页翻页       | 点击下一页 | 正确加载下一页数据 | P1 | ⏳ |
...
| AC-GW-002    | 断开重连     | 模拟网络断开 | 自动重连，最多 5 次 | P0 | ⏳ |
| AC-GW-003    | RPC 请求响应 | 发送 JSON-RPC 请求 | 收到正确响应 | P0 | ⏳ |
| AC-GW-004    | 请求超时     | 模拟无响应 | 超时后 reject | P1 | ⏳ |
| AC-GW-005    | 事件订阅     | 订阅事件 | 收到服务端推送 | P1 | ⏳ |
...
| WA-AUTH-004  | 非管理员访问管理页 | 普通用户访问 /admin | 拒绝访问或跳转 | P0 | ⏳ |
| WA-AUTH-005  | 注册跳转       | 点击注册链接 | 跳转登录页（当前实现） | P1 | ⏳ |
...
| WA-DEV-002   | 设备详情   | 点击设备 | 显示设备信息与状态 | P0 | ⏳ |
| WA-DEV-003   | 设备状态   | 查看在线/离线状态 | 状态正确显示 | P1 | ⏳ |

### 6.2.3 技能商店与我的技能

| 用例ID       | 场景           | 操作步骤 | 预期结果 | 优先级 | 状态 |
| ------------ | -------------- | -------- | -------- | ------ | ---- |
| WA-SKILL-001 | 商店浏览       | 进入技能商店 | 显示可用技能列表 | P0 | ⏳ |
| WA-SKILL-002 | 安装技能       | 点击安装 | 技能安装成功，出现在我的技能 | P0 | ⏳ |
| WA-SKILL-003 | 卸载技能       | 点击卸载 | 技能从我的技能移除 | P0 | ⏳ |
| WA-SKILL-004 | 我的技能列表   | 进入我的技能 | 显示已安装技能 | P0 | ⏳ |
...
| WA-GW-002    | 断开重连   | 模拟断网 | 自动重连 | P0 | ⏳ |
| WA-GW-003    | RPC 错误处理 | 发送错误请求 | 显示友好错误提示 | P1 | ⏳ |
...
| gateway/e2e (21文件)  | *.e2e.test.ts (21个)    | 114    | 103  | 9    | 🟡   |

## 7.2 应用功能测试进度

| 应用           | 总用例（约） | P0 | P1 | P2 | 已通过 | 状态 |
| -------------- | ------------ | -- | -- | -- | ------ | ---- |
| admin-console  | 37           | 14 | 14 | 9  | 0      | ⏳   |
| web-admin      | 22           | 11 | 8  | 3  | 0      | ⏳   |
| windows 代码   | 21           | 12 | 9  | 0  | 0      | ⏳   |
| windows UI     | 14           | 7  | 5  | 2  | 0      | ⏳   |
...
| 8  | Gateway E2E | server.health: presence includes client fingerprint 断言失败 | ❌ 待修复 (Windows 环境) |
| 9  | Gateway E2E | server.hooks/reload/chat-b: 测试内部 timeout 5s 不够 | ❌ 待修复 (需加大 testTimeout) |
| 10 | Gateway E2E | server.sessions-send: loopback auth token mismatch (Windows 路径) | ❌ 待修复 (Windows 兼容) |
| 11 | Gateway E2E | server.chat: agentCommand 引用未定义 (ReferenceError) | ❌ 待修复 (test scope 问题) |
...
**阶段 B — 前后端联调测试**
- [ ] admin-console P0 用例通过（AC-AUTH + AC-USER + AC-SUB + AC-GW）
- [ ] web-admin P0 用例通过（WA-AUTH + WA-DEV + WA-SKILL + WA-SUB + WA-GW）
- [ ] windows 代码模块 P0 通过（WIN-GW + WIN-PAIR + WIN-SKILL + WIN-SEC）

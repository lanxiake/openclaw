...
export function triggerMtBotRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.MTBOT_SYSTEMD_UNIT,
        process.env.MTBOT_PROFILE,
      );
  ...
  const label =
    process.env.MTBOT_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.MTBOT_PROFILE);

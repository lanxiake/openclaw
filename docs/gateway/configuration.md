---
summary: "All configuration options for ~/.mtbot/mtbot.json with examples"
read_when:
...
MtBot reads an optional **JSON5** config from `~/.mtbot/mtbot.json` (comments + trailing commas allowed).

If the file is missing, MtBot uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.mtbot/workspace`). You usually only need a config to:
...
MtBot only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `mtbot doctor`, `mtbot logs`, `mtbot health`, `mtbot status`, `mtbot service`, `mtbot help`).
- Run `mtbot doctor` to see the exact issues.
- Run `mtbot doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `mtbot config set`. Keep a backup of `~/.mtbot/mtbot.json`.
...
```bash
mtbot gateway call config.get --params '{}' # capture payload.hash
mtbot gateway call config.apply --params '{
...
```bash
mtbot gateway call config.get --params '{}' # capture payload.hash
mtbot gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.mtbot/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@mtbot", "reisponde"] },
      },
    ],
  },
  channels: {
...
// ~/.mtbot/agents.json5
{
  ...
  list: [{ id: "main", workspace: "~/.mtbot/workspace" }],
}
```
...
{ "$include": "/etc/mtbot/base.json5" } // absolute
...
```json5
// ~/.mtbot/clients/mueller/agents.json5
[
...
```json5
// ~/.mtbot/clients/mueller/broadcast.json5
{
...
MtBot reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, MtBot runs your login shell and imports only the missing expected keys (never overrides).
...
- `MTBOT_LOAD_SHELL_ENV=1`
- `MTBOT_SHELL_ENV_TIMEOUT_MS=15000`
...
MtBot stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.mtbot/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.mtbot/credentials/oauth.json` (or `$MTBOT_STATE_DIR/credentials/oauth.json`)
...
- `~/.mtbot/agent/*` (migrated by `mtbot doctor` into `~/.mtbot/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `MTBOT_OAUTH_DIR`
- Agent dir (default agent root override): `MTBOT_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, MtBot imports `oauth.json` entries into `auth-profiles.json`.
...
If set, MtBot derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/mtbot/mtbot-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/mtbot/mtbot.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.mtbot/credentials/whatsapp/biz
          // authDir: "~/.mtbot/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `mtbot doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@mtbot", "mtbot"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@mtbot"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.mtbot/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.mtbot/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.mtbot/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.mtbot/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.mtbot/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.mtbot/workspace-home" },
      { id: "work", workspace: "~/.mtbot/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `mtbot.json`).
...
MtBot starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["mtbot-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-mtbot",
...
MtBot starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `mtbot plugins install @mtbot/mattermost` (or `./extensions/mattermost` from a git checkout).
...
MtBot starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
MtBot spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, MtBot
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/mtbot" } },
}
```
...
When a file exceeds this limit, MtBot logs a warning and injects a truncated
...
message envelopes). If unset, MtBot uses the host timezone at runtime.
...
`[mtbot]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[mtbot]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[mtbot]"`, MtBot will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, MtBot generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.mtbot/settings/tts.json",
      elevenlabs: {
...
MtBot also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, MtBot currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `mtbot configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:mtbot`: all built-in MtBot tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.mtbot/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.mtbot/sandboxes",
        docker: {
          image: "mtbot-sandbox:bookworm-slim",
          containerPrefix: "mtbot-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
        ...
        browser: {
          enabled: false,
          image: "mtbot-sandbox-browser:bookworm-slim",
          containerPrefix: "mtbot-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
MtBot uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.mtbot/agents/<agentId>/agent/models.json` or by defining the same schema inside your
MtBot config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, MtBot writes/merges a `models.json` into
`~/.mtbot/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. MtBot uses
...
- Shortcut: `mtbot onboard --auth-choice opencode-zen`.
...
Shortcut: `mtbot onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `mtbot onboard --auth-choice moonshot-api-key`.
...
- Set `KIMI_API_KEY` in the environment or use `mtbot onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `mtbot onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `mtbot onboard --auth-choice minimax-api`.
...
- Override the agent config root with `MTBOT_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.mtbot/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.mtbot/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.mtbot/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, MtBot stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.mtbot/skills` (workspace skills
...
from `~/.mtbot/extensions`, `<workspace>/.mtbot/extensions`, plus any
...
### `browser` (mtbot-managed browser)

MtBot can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for mtbot and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (MtBot.app menubar, or `mtbot gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      mtbot: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/mtbot" }
...
- When a connection comes from one of these IPs, MtBot uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `mtbot gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `MTBOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `MTBOT_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. MtBot
...
- MtBot.app watches `~/.mtbot/mtbot.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.mtbot/mtbot.json` (or `MTBOT_CONFIG_PATH`) and applies changes automatically.
...
- `~/.mtbot/mtbot.json` (or `MTBOT_CONFIG_PATH`)
...
- `MTBOT_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `mtbot --dev …` → uses `~/.mtbot-dev` + shifts ports from base `19001`
- `mtbot --profile <name> …` → uses `~/.mtbot-<name>` (port via config/env/flags)
...
```bash
MTBOT_CONFIG_PATH=~/.mtbot/a.json \
MTBOT_STATE_DIR=~/.mtbot-a \
mtbot gateway --port 19001
```
...
Gmail helper config (used by `mtbot webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "mtbot@gmail.com",
...
- Set `MTBOT_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, MtBot defaults `serve.path` to `/` so
...
Default root: `~/.mtbot/workspace/canvas`  
Default port: `18793` (chosen to avoid the mtbot browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__mtbot__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__mtbot__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.mtbot/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.mtbot/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.mtbot/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_mtbot-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `mtbot` (advertises `mtbot.local`). Override with `MTBOT_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_mtbot-gw._tcp` under `~/.mtbot/dns/` using the configured discovery domain (example: `mtbot.internal.`).

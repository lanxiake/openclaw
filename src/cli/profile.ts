...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.mtbot${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.MTBOT_PROFILE = profile;

  const stateDir = env.MTBOT_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.MTBOT_STATE_DIR?.trim()) {
    env.MTBOT_STATE_DIR = stateDir;
  }

  if (!env.MTBOT_CONFIG_PATH?.trim()) {
    env.MTBOT_CONFIG_PATH = path.join(stateDir, "mtbot.json");
  }

  if (profile === "dev" && !env.MTBOT_GATEWAY_PORT?.trim()) {
    env.MTBOT_GATEWAY_PORT = "19001";
  }
}

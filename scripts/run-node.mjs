...
const compilerOverride = env.MTBOT_TS_COMPILER ?? env.CLAWDBOT_TS_COMPILER;
...
const logRunner = (message) => {
  if (env.MTBOT_RUNNER_LOG === "0") {
    return;
  }
  process.stderr.write(`[mtbot] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["mtbot.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });

...
import { __testing, createMtBotCodingTools } from "./pi-tools.js";
...
describe("createMtBotCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createMtBotCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mtbot-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "mtbot-outside.txt");

...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.MTBOT_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.MTBOT_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.MTBOT_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[mtbot] ${parsed.error}`);
    process.exit(2);
  }

...
`;
    ...
    expect(parsed.mtbot.emoji).toBe("💾");
    expect(parsed.mtbot.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.mtbot.emoji).toBe("📝");
    expect(parsed.mtbot.events).toEqual(["command"]);
    expect(parsed.mtbot.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.mtbot.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"mtbot": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"mtbot": {"events": ["test"]}}');
  });
...
describe("resolveMtBotMetadata", () => {
  it("extracts mtbot metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveMtBotMetadata(frontmatter);
  ...
  it("returns undefined when mtbot key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        mtbot: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with MtBot" },
            { id: "npm", kind: "npm", package: "@mtbot/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@mtbot/hook");
  });
...
`;
    ...
    const mtbot = resolveMtBotMetadata(frontmatter);
    expect(mtbot).toBeDefined();
    expect(mtbot?.emoji).toBe("💾");
    expect(mtbot?.events).toEqual(["command:new"]);
    expect(mtbot?.requires?.config).toEqual(["workspace.dir"]);
    expect(mtbot?.install?.[0].kind).toBe("bundled");
  });

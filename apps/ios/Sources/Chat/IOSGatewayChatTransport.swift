...
struct IOSGatewayChatTransport: MtBotChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> MtBotChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(MtBotChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> MtBotChatHistoryPayload {
        ...
        return try JSONDecoder().decode(MtBotChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [MtBotChatAttachmentPayload]) async throws -> MtBotChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [MtBotChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(MtBotChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(MtBotGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MtBotGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MtBotChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MtBotAgentEventPayload.self)
                        {

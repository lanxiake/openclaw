...
read_when:
  - You are building a MtBot plugin
  - You need to ship a plugin config schema or debug plugin validation errors
---

# Plugin manifest (mtbot.plugin.json)

Every plugin **must** ship a `mtbot.plugin.json` file in the **plugin root**.
MtBot uses this manifest to validate configuration **without executing plugin

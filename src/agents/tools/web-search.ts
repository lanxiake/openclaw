...
type WebSearchConfig = NonNullable<MtBotConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: MtBotConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("mtbot configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.mtbot.top/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://mtbot.top",
      "X-Title": "MtBot Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [

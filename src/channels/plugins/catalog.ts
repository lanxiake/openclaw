...
import { discoverMtBotPlugins } from "../../plugins/discovery.js";
import type { PluginOrigin } from "../../plugins/types.js";
import type { MtBotPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, MtBotPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["MTBOT_PLUGIN_CATALOG_PATHS", "MTBOT_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<MtBotPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: MtBotPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverMtBotPlugins({ workspaceDir: options.workspaceDir });

...
export async function installCompletion(shell: string, yes: boolean, binName = "mtbot") {
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c mtbot -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'mtbot gateway' and not 'mtbot other gateway', we need to check the sequence.

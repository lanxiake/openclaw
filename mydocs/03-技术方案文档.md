# AI 个人助理平台 - 技术方案文档 v2.0

> 版本: 2.0 | 创建日期: 2026-02-06 | 状态: 基于现有架构重构

---

## 1. 技术选型 (复用现有)

### 1.1 后端技术栈

| 层级      | 技术          | 复用来源 | 说明                    |
| --------- | ------------- | -------- | ----------------------- |
| 运行时    | Node.js 22+   | ✅ 现有  | 与 OpenClaw 一致        |
| Web 框架  | Fastify 4.x   | ✅ 现有  | Gateway HTTP 服务       |
| WebSocket | ws 8.x        | ✅ 现有  | 实时通信                |
| 类型验证  | TypeBox       | ✅ 现有  | Schema 定义和运行时验证 |
| 配置管理  | YAML + Zod    | ✅ 现有  | 配置加载和验证          |
| AI 服务   | Claude API    | ✅ 现有  | Pi Agent Core 集成      |
| 数据库    | PostgreSQL 16 | 🆕 新增  | 用户、订阅数据          |
| ORM       | Drizzle ORM   | 🆕 新增  | 类型安全 ORM            |

### 1.2 前端/客户端技术栈

| 类型     | 技术         | 复用来源 | 说明             |
| -------- | ------------ | -------- | ---------------- |
| Web 框架 | React 18+    | 🆕 新增  | 管理后台         |
| 构建工具 | Vite 5.x     | 🆕 新增  | 快速构建         |
| UI 库    | shadcn/ui    | 🆕 新增  | TailwindCSS 集成 |
| 桌面框架 | Electron 28+ | 🆕 新增  | Windows 客户端   |
| 状态管理 | Zustand      | 🆕 新增  | 轻量状态管理     |

---

## 2. 核心模块复用详解

### 2.1 Gateway 服务 (100% 复用)

**来源**: `src/gateway/`

```
Gateway 核心模块
├── server.impl.ts          # 服务器初始化 ✅ 直接复用
├── server-methods.ts       # RPC 方法注册 ✅ 扩展
├── server-ws-runtime.ts    # WebSocket 运行时 ✅ 直接复用
├── protocol/               # 协议定义 ✅ 扩展
│   └── schema/             # TypeBox Schema ✅ 扩展
├── auth.ts                 # 认证 ✅ 直接复用
└── config-reload.ts        # 配置热重载 ✅ 直接复用
```

**扩展方式**:

```typescript
// 新增 assistant.* RPC 方法，不修改现有代码
// 位置: src/gateway/server-methods/assistant.ts

import { Type } from "@sinclair/typebox";

export const AssistantFileListParams = Type.Object({
  path: Type.String(),
  recursive: Type.Optional(Type.Boolean()),
});

export async function assistantFileList(params: Static<typeof AssistantFileListParams>) {
  // 实现...
}

// 注册到 server-methods.ts
export const assistantMethods = {
  "assistant.fileList": assistantFileList,
  "assistant.fileRead": assistantFileRead,
  "assistant.fileWrite": assistantFileWrite,
  // ...
};
```

### 2.2 AI 代理系统 (90% 复用)

**来源**: `src/agents/`

```
代理系统模块
├── agent-scope.ts          # 代理作用域 ✅ 直接复用
├── auth-profiles.ts        # 认证配置 ✅ 直接复用
├── skills/                 # 技能框架 ✅ 扩展
│   ├── types.ts            # 技能类型 ✅ 扩展
│   ├── refresh.ts          # 技能热重载 ✅ 复用
│   └── install.ts          # 技能安装 ✅ 复用
└── tools/                  # 工具系统 ✅ 扩展
    └── common.ts           # 通用工具函数 ✅ 直接复用
```

**复用工具函数**:

```typescript
// 来自 src/agents/tools/common.ts
export function readStringParam(params, key, options) { ... }
export function readNumberParam(params, key, options) { ... }
export function readBooleanParam(params, key, options) { ... }
export function sanitizeToolResultImages(images) { ... }

// 在新工具中复用
import { readStringParam } from '@/agents/tools/common'

export const fileListTool = {
  name: 'file_list',
  async execute(params) {
    const path = readStringParam(params, 'path', { required: true })
    // ...
  }
}
```

### 2.3 插件系统 (作为技能系统基础)

**来源**: `src/plugins/`

```
插件系统
├── discovery.ts            # 插件发现 ✅ 复用模式
├── loader.ts               # 动态加载 ✅ 复用模式
├── runtime.ts              # 运行时管理 ✅ 复用模式
└── types.ts                # 插件接口 ✅ 扩展
```

**技能系统扩展**:

```typescript
// 基于 OpenClawPluginService 扩展
interface AssistantSkill extends OpenClawPluginService {
  type: "server" | "local";

  // 复用现有元数据
  metadata: OpenClawSkillMetadata;

  // 新增：订阅信息
  subscription?: {
    required: boolean;
    price?: number;
    period?: "monthly" | "yearly" | "once";
  };

  // 技能执行入口
  execute: (context: SkillContext) => Promise<SkillResult>;
}
```

### 2.4 配置系统 (100% 复用)

**来源**: `src/config/`

```
配置系统
├── config.ts               # 配置加载 ✅ 直接复用
├── types.*.ts              # 类型定义 ✅ 扩展
├── sessions/               # 会话管理 ✅ 直接复用
└── migrations/             # 配置迁移 ✅ 直接复用
```

**扩展配置**:

```yaml
# ~/.openclaw/config.yaml 新增配置节
assistant:
  enabled: true
  skills:
    serverEndpoint: "https://skills.example.com"
    localPath: "~/.openclaw/skills"

  # 敏感操作确认级别
  confirmLevel: "medium" # low | medium | high

  # Windows 客户端配置
  windows:
    autoStart: true
    trayIcon: true
```

### 2.5 基础设施层 (100% 复用)

**来源**: `src/infra/`

| 模块     | 文件                  | 用途         | 复用方式 |
| -------- | --------------------- | ------------ | -------- |
| 设备配对 | `device-pairing.ts`   | 二维码配对   | 直接复用 |
| 执行审批 | `exec-approvals.ts`   | 敏感操作确认 | 直接复用 |
| 重试逻辑 | `retry.ts`            | 网络重试     | 直接复用 |
| 心跳     | `heartbeat-runner.ts` | 连接保活     | 直接复用 |
| 端口管理 | `ports.ts`            | 端口分配     | 直接复用 |

---

## 3. 新增模块设计

### 3.1 Windows 客户端架构

```
apps/windows/
├── src/
│   ├── main/                       # Electron 主进程
│   │   ├── index.ts                # 入口
│   │   ├── gateway-client.ts       # WebSocket 客户端
│   │   ├── skill-executor.ts       # 技能沙箱执行
│   │   ├── system-service.ts       # 系统服务封装
│   │   │   ├── file-ops.ts         # 文件操作
│   │   │   ├── process-ops.ts      # 进程操作
│   │   │   └── system-info.ts      # 系统信息
│   │   ├── tray-manager.ts         # 系统托盘
│   │   └── auto-updater.ts         # 自动更新
│   │
│   ├── renderer/                   # 渲染进程 (React)
│   │   ├── App.tsx
│   │   ├── components/
│   │   │   ├── ChatPanel/          # 对话面板
│   │   │   ├── ConfirmDialog/      # 操作确认
│   │   │   └── SkillStore/         # 技能商店
│   │   └── hooks/
│   │       └── useGateway.ts       # Gateway 连接 Hook
│   │
│   └── preload/                    # 预加载脚本
│       └── index.ts                # IPC 桥接
│
├── electron-builder.json           # 打包配置
└── package.json
```

### 3.2 GatewayClient 实现

```typescript
// apps/windows/src/main/gateway-client.ts
import WebSocket from "ws";
import { EventEmitter } from "events";

// 复用协议类型
import type { Message, MessageType } from "@/gateway/protocol";

export class GatewayClient extends EventEmitter {
  private ws: WebSocket | null = null;
  private reconnectTimer: NodeJS.Timer | null = null;
  private pendingRequests = new Map<string, { resolve; reject; timeout }>();

  constructor(private config: GatewayClientConfig) {
    super();
  }

  async connect(): Promise<void> {
    const { url, token } = this.config;

    this.ws = new WebSocket(url, {
      headers: { Authorization: `Bearer ${token}` },
    });

    this.ws.on("open", () => this.handleOpen());
    this.ws.on("message", (data) => this.handleMessage(data));
    this.ws.on("close", () => this.handleClose());
    this.ws.on("error", (err) => this.handleError(err));
  }

  // RPC 调用 (复用现有协议格式)
  async call<T>(method: string, params: unknown): Promise<T> {
    const id = crypto.randomUUID();
    const message: Message = {
      type: "req",
      id,
      method,
      params,
    };

    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        this.pendingRequests.delete(id);
        reject(new Error("Request timeout"));
      }, 30000);

      this.pendingRequests.set(id, { resolve, reject, timeout });
      this.send(message);
    });
  }

  // 文件操作快捷方法
  async fileList(path: string): Promise<FileInfo[]> {
    return this.call("assistant.fileList", { path });
  }

  async fileRead(path: string): Promise<string> {
    return this.call("assistant.fileRead", { path });
  }

  // ... 更多方法
}
```

### 3.3 技能沙箱执行器

```typescript
// apps/windows/src/main/skill-executor.ts
import { VM } from "vm2";
import { Worker } from "worker_threads";

export class SkillExecutor {
  private runningSkills = new Map<string, Worker>();

  constructor(private config: SkillExecutorConfig) {}

  // 执行服务端技能 (代码从 Gateway 动态加载)
  async executeServerSkill(
    skillCode: string,
    params: unknown,
    context: SkillContext,
  ): Promise<SkillResult> {
    // 在隔离的 Worker 中执行
    return new Promise((resolve, reject) => {
      const worker = new Worker(
        `
        const { parentPort, workerData } = require('worker_threads')
        const vm = require('vm2')

        const sandbox = new vm.VM({
          timeout: ${this.config.timeout || 30000},
          sandbox: {
            // 安全的 API 白名单
            console: { log: (...args) => parentPort.postMessage({ type: 'log', args }) },
            fs: workerData.safeFs,
            system: workerData.safeSystem,
            ui: workerData.safeUi,
          }
        })

        try {
          const result = sandbox.run(workerData.code)
          parentPort.postMessage({ type: 'result', data: result })
        } catch (err) {
          parentPort.postMessage({ type: 'error', message: err.message })
        }
        `,
        {
          eval: true,
          workerData: {
            code: skillCode,
            params,
            safeFs: this.createSafeFs(context),
            safeSystem: this.createSafeSystem(context),
            safeUi: this.createSafeUi(context),
          },
        },
      );

      worker.on("message", (msg) => {
        if (msg.type === "result") resolve(msg.data);
        if (msg.type === "error") reject(new Error(msg.message));
      });

      // 超时保护
      setTimeout(() => {
        worker.terminate();
        reject(new Error("Skill execution timeout"));
      }, this.config.timeout || 30000);
    });
  }

  // 创建安全的文件系统 API
  private createSafeFs(context: SkillContext) {
    return {
      readFile: async (path: string) => {
        if (!this.isPathAllowed(path, context)) {
          throw new Error("Access denied");
        }
        // 实现...
      },
      writeFile: async (path: string, content: string) => {
        if (!this.isPathAllowed(path, context)) {
          throw new Error("Access denied");
        }
        // 需要用户确认
        const approved = await context.requestConfirm({
          action: "file_write",
          path,
          preview: content.slice(0, 100),
        });
        if (!approved) throw new Error("User denied");
        // 实现...
      },
      // ...
    };
  }
}
```

### 3.4 系统工具集

```typescript
// packages/assistant-tools/file-manager/index.ts
import { Type, Static } from "@sinclair/typebox";
import { readStringParam, readBooleanParam } from "@/agents/tools/common";
import type { AnyAgentTool } from "@/agents/tools/types";
import fs from "fs/promises";
import path from "path";

// Schema 定义 (复用 TypeBox 模式)
const FileListParamsSchema = Type.Object({
  path: Type.String({ description: "目录路径" }),
  recursive: Type.Optional(Type.Boolean({ default: false })),
  filter: Type.Optional(Type.String({ description: "文件名过滤 (glob)" })),
});

// 工具实现
export const fileListTool: AnyAgentTool = {
  name: "file_list",
  description: "列出指定目录下的文件和文件夹",
  parameters: FileListParamsSchema,

  async execute(params: Static<typeof FileListParamsSchema>, context) {
    const dirPath = readStringParam(params, "path", { required: true });
    const recursive = readBooleanParam(params, "recursive") ?? false;

    // 安全检查：禁止访问系统目录
    if (isSystemPath(dirPath)) {
      throw new Error("Access to system directories is not allowed");
    }

    const entries = await fs.readdir(dirPath, { withFileTypes: true });
    const results: FileInfo[] = [];

    for (const entry of entries) {
      const fullPath = path.join(dirPath, entry.name);
      const stat = await fs.stat(fullPath);

      results.push({
        name: entry.name,
        path: fullPath,
        type: entry.isDirectory() ? "directory" : "file",
        size: stat.size,
        modified: stat.mtime.toISOString(),
      });

      if (recursive && entry.isDirectory()) {
        // 递归获取子目录
        const subResults = await this.execute({ path: fullPath, recursive: true }, context);
        results.push(...subResults);
      }
    }

    return results;
  },
};

// 工具集导出
export const fileManagerTools = [
  fileListTool,
  fileReadTool,
  fileWriteTool,
  fileMoveTool,
  fileDeleteTool,
  fileCopyTool,
  fileSearchTool,
];
```

---

## 4. 通信协议扩展

### 4.1 新增消息类型

```typescript
// src/gateway/protocol/schema/assistant.ts
import { Type } from "@sinclair/typebox";

// 文件操作
export const AssistantFileListParams = Type.Object({
  path: Type.String(),
  recursive: Type.Optional(Type.Boolean()),
});

export const AssistantFileReadParams = Type.Object({
  path: Type.String(),
  encoding: Type.Optional(Type.String({ default: "utf-8" })),
});

export const AssistantFileWriteParams = Type.Object({
  path: Type.String(),
  content: Type.String(),
  encoding: Type.Optional(Type.String({ default: "utf-8" })),
});

// 技能操作
export const AssistantSkillLoadParams = Type.Object({
  skillId: Type.String(),
  version: Type.Optional(Type.String()),
});

export const AssistantSkillExecuteParams = Type.Object({
  skillId: Type.String(),
  params: Type.Unknown(),
});

// 操作确认
export const AssistantConfirmRequestParams = Type.Object({
  action: Type.String(),
  description: Type.String(),
  details: Type.Optional(Type.Unknown()),
  level: Type.Union([Type.Literal("low"), Type.Literal("medium"), Type.Literal("high")]),
});
```

### 4.2 RPC 方法注册

```typescript
// src/gateway/server-methods/assistant.ts
export const assistantMethods = {
  // 文件操作
  "assistant.fileList": handleFileList,
  "assistant.fileRead": handleFileRead,
  "assistant.fileWrite": handleFileWrite,
  "assistant.fileMove": handleFileMove,
  "assistant.fileDelete": handleFileDelete,
  "assistant.fileCopy": handleFileCopy,
  "assistant.fileSearch": handleFileSearch,

  // 系统操作
  "assistant.systemInfo": handleSystemInfo,
  "assistant.processList": handleProcessList,
  "assistant.processKill": handleProcessKill,

  // 技能操作
  "assistant.skillList": handleSkillList,
  "assistant.skillLoad": handleSkillLoad,
  "assistant.skillExecute": handleSkillExecute,
  "assistant.skillUnload": handleSkillUnload,

  // 操作确认
  "assistant.confirmRequest": handleConfirmRequest,
  "assistant.confirmResponse": handleConfirmResponse,
};
```

---

## 5. 安全方案 (复用 + 增强)

### 5.1 复用现有安全机制

| 机制     | 来源                          | 说明         |
| -------- | ----------------------------- | ------------ |
| TLS 加密 | Gateway 配置                  | 所有通信加密 |
| JWT 认证 | `src/gateway/auth.ts`         | Token 认证   |
| 设备配对 | `src/infra/device-pairing.ts` | 安全配对     |
| 执行审批 | `src/infra/exec-approvals.ts` | 敏感操作确认 |

### 5.2 新增安全机制

**技能沙箱安全**:

```typescript
// 资源限制
const sandboxConfig = {
  timeout: 30000, // 30 秒超时
  memoryLimit: 256 * 1024 * 1024, // 256MB 内存

  // API 白名单
  allowedApis: [
    "fs.readFile",
    "fs.writeFile",
    "fs.readdir",
    "system.getCpuUsage",
    "system.getMemoryUsage",
    "ui.confirm",
    "ui.notify",
  ],

  // 文件系统限制
  allowedPaths: ["~/Documents", "~/Downloads", "~/Desktop"],

  // 禁止访问
  deniedPaths: ["C:\\Windows", "C:\\Program Files", "~/.ssh", "~/.gnupg"],
};
```

**敏感操作分级** (复用 `exec-approvals.ts` 模式):

```typescript
const operationLevels = {
  // 低风险：直接执行
  low: ["file_list", "file_read", "system_info", "process_list"],

  // 中风险：单次确认
  medium: ["file_write", "file_move", "file_copy", "app_launch"],

  // 高风险：二次确认
  high: ["file_delete", "process_kill", "system_config"],

  // 禁止：拒绝执行
  blocked: ["format_disk", "registry_edit", "system_restore"],
};
```

---

## 6. 部署方案

### 6.1 Gateway 部署 (复用现有)

现有 OpenClaw Gateway 部署方式完全适用：

```bash
# 本地运行
openclaw gateway run --bind loopback --port 18789

# Docker 部署
docker run -d \
  -p 18789:18789 \
  -v ~/.openclaw:/root/.openclaw \
  openclaw/gateway
```

### 6.2 Windows 客户端分发

| 分发方式 | 说明                |
| -------- | ------------------- |
| 官网下载 | NSIS 安装包 (.exe)  |
| 便携版   | 免安装压缩包 (.zip) |
| 自动更新 | electron-updater    |

```json
// electron-builder.json
{
  "appId": "com.openclaw.assistant",
  "productName": "OpenClaw Assistant",
  "win": {
    "target": ["nsis", "zip"],
    "icon": "assets/icon.ico"
  },
  "nsis": {
    "oneClick": false,
    "allowToChangeInstallationDirectory": true,
    "createDesktopShortcut": true,
    "createStartMenuShortcut": true
  },
  "publish": {
    "provider": "generic",
    "url": "https://releases.openclaw.ai/assistant"
  }
}
```

---

## 7. 项目结构总结

```
openclaw/
├── src/                              # 核心代码 (不修改)
│   ├── gateway/                      # ✅ 复用
│   ├── agents/                       # ✅ 复用
│   ├── plugins/                      # ✅ 复用
│   ├── config/                       # ✅ 复用
│   ├── routing/                      # ✅ 复用
│   ├── infra/                        # ✅ 复用
│   └── ...
│
├── packages/                         # 🆕 新增包
│   ├── assistant-skills/             # 技能系统
│   ├── assistant-tools/              # Windows 工具
│   └── assistant-shared/             # 共享代码
│
├── apps/
│   ├── macos/                        # ✅ 现有
│   ├── ios/                          # ✅ 现有
│   ├── android/                      # ✅ 现有
│   └── windows/                      # 🆕 新增
│
└── skills/                           # 🆕 技能库
    ├── server/
    └── templates/
```

---

_— 文档结束 —_

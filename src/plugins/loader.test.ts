...
import { loadMtBotPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.MTBOT_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `mtbot-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "mtbot.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.MTBOT_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadMtBotPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.MTBOT_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadMtBotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadMtBotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],

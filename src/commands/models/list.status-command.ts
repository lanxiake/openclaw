...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveMtBotAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("mtbot models auth setup-token")}\` or \`${formatCliCommand("mtbot configure")}\`.`
          : `Run \`${formatCliCommand("mtbot configure")}\` or set an API key env var.`;

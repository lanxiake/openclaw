...
export async function updateConfig(
  mutator: (cfg: MtBotConfig) => MtBotConfig,
): Promise<MtBotConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: MtBotConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: MtBotConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("mtbot agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}

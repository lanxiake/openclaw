...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "MTBOT_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "mtbot-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.MTBOT_STATE_DIR = path.join(home, ".mtbot");
    ...
    await fs.mkdir(path.join(home, ".mtbot"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.mtbot${path.sep}media`);

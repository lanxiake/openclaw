...
This directory contains hooks that ship with MtBot. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.mtbot.top/hooks/soul-evil
...
Hooks can be configured in `~/.mtbot/mtbot.json`:
...
2. Restart gateway: `pkill -9 -f 'mtbot.*gateway' && pnpm mtbot gateway`
3. Enable the hook: `mtbot hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.mtbot.top/hooks

...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `mtbot sandbox explain`.
...
Legacy `agent.*` configs are migrated by `mtbot doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=mtbot-sbx-"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${MTBOT_STATE_DIR:-$HOME/.mtbot}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```

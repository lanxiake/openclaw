...
docker run --rm -t \
  ...
  -e MTBOT_LIVE_TEST=1 \
  -e MTBOT_LIVE_GATEWAY_MODELS="${MTBOT_LIVE_GATEWAY_MODELS:-${CLAWDBOT_LIVE_GATEWAY_MODELS:-all}}" \
  -e MTBOT_LIVE_GATEWAY_PROVIDERS="${MTBOT_LIVE_GATEWAY_PROVIDERS:-${CLAWDBOT_LIVE_GATEWAY_PROVIDERS:-}}" \
  -e MTBOT_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${MTBOT_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-${CLAWDBOT_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-}}" \

...
enum DebugActions {
    private static let verboseDefaultsKey = "mtbot.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "mtbot.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = MtBotPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "MtBot", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = MtBotPaths.configURL
        guard

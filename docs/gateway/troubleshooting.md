---
summary: "Quick troubleshooting guide for common MtBot failures"
read_when:
...
When MtBot misbehaves, here's how to fix it.
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| `mtbot status`                  | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview                       |
| `mtbot status --all`            | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail                                   | When you need to share a debug report             |
| `mtbot status --deep`           | Runs gateway health checks (incl. provider probes; requires reachable gateway)                         | When “configured” doesn’t mean “working”          |
| `mtbot gateway probe`           | Gateway discovery + reachability (local + remote targets)                                              | When you suspect you’re probing the wrong gateway |
| `mtbot channels status --probe` | Asks the running gateway for channel status (and optionally probes)                                    | When gateway is reachable but channels misbehave  |
| `mtbot gateway status`          | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error                      | When the service “looks loaded” but nothing runs  |
| `mtbot logs --follow`           | Live logs (best signal for runtime issues)                                                             | When you need the actual failure reason           |

**Sharing output:** prefer `mtbot status --all` (it redacts tokens). If you paste `mtbot status`, consider setting `MTBOT_SHOW_SECRETS=0` first (token previews).
...
- Preferred: `mtbot logs --follow`
- File logs (always): `/tmp/mtbot/mtbot-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$MTBOT_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u mtbot-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  mtbot config set gateway.mode local
  ```
...
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  mtbot config set gateway.mode remote
  mtbot config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `mtbot setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.mtbot/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `mtbot doctor`
...
- then run `mtbot sandbox recreate --agent <id>` (or `--all`)
...
- `gateway.mode` must be `local` for `mtbot gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `mtbot gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `mtbot gateway status` and `mtbot doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `MTBOT_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `mtbot gateway status` shows a config mismatch**
...
- Fix: rerun `mtbot gateway install --force` from the same `--profile` / `MTBOT_STATE_DIR` you want the service to use.

**If `mtbot gateway status` reports service config issues**

- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `mtbot doctor` to update it (or `mtbot gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `MTBOT_GATEWAY_TOKEN`) and restart the service.

**If `mtbot gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/mtbot` on disk.
...
Symptoms: `pwd` or file tools show `~/.mtbot/sandboxes/...` even though you
...
MtBot intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `mtbot models list` or
  `mtbot models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${MTBOT_CONFIG_PATH:-$HOME/.mtbot/mtbot.json}"
```
...
tail -f "$(ls -t /tmp/mtbot/mtbot-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
mtbot logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.mtbot/agents/<agentId>/sessions/
```
...
mtbot logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
mtbot gateway --verbose
```

If you’re logged out / unlinked:

```bash
mtbot channels logout
trash "${MTBOT_STATE_DIR:-$HOME/.mtbot}/credentials" # if logout can't cleanly remove everything
mtbot channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/mtbot/mtbot-*.log | head -1)" | tail -20
```

### High Memory Usage

MtBot keeps conversation history in memory.
...
MtBot now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
- `mtbot doctor` reports every invalid entry.
- `mtbot doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `mtbot logs`, `mtbot health`, `mtbot status`, `mtbot gateway status`, and `mtbot gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/mtbot/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `mtbot models status` (CLI).
...
```bash
curl -fsSL https://mtbot.bot/install.sh | bash -s -- --install-method git --no-onboard
```
...
5. Run `mtbot channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. MtBot scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/mtbot/mtbot/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
mtbot gateway status
mtbot gateway stop
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.mtbot.* still works)
```
...
Ensure the global `mtbot` CLI is installed and matches the app version:
...
#   ${MTBOT_CONFIG_PATH:-$HOME/.mtbot/mtbot.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
mtbot gateway --verbose
mtbot channels login --verbose
```
...
| Log                               | Location                                                                                                                                                                                                                                                                                                                    |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway file logs (structured)    | `/tmp/mtbot/mtbot-YYYY-MM-DD.log` (or `logging.file`)                                                                                                                                                                                                                                                                 |
| Gateway service logs (supervisor) | macOS: `$MTBOT_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.mtbot/logs/...`; profiles use `~/.mtbot-<profile>/logs/...`)<br />Linux: `journalctl --user -u mtbot-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "MtBot Gateway (<profile>)" /V /FO LIST` |
| Session files                     | `$MTBOT_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                            |
| Media cache                       | `$MTBOT_STATE_DIR/media/`                                                                                                                                                                                                                                                                                                |
| Credentials                       | `$MTBOT_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                                          |
...
# mtbot gateway uninstall

trash "${MTBOT_STATE_DIR:-$HOME/.mtbot}"
mtbot channels login         # re-pair WhatsApp
mtbot gateway restart           # or: mtbot gateway
```
...
1. Check logs first: `/tmp/mtbot/` (default: `mtbot-YYYY-MM-DD.log`, or your configured `logging.file`)

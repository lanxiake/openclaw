...
import { scheduleGatewaySigusr1Restart, triggerMtBotRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting MtBot in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerMtBotRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting MtBot via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};

...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: MtBotOAuthStore.AnthropicOAuthStatus = MtBotOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: MtBotOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = MtBotOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = MtBotOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try MtBotOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. MtBot can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: MtBotOAuthStore.AnthropicOAuthStatus,

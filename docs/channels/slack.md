...
3. Set tokens for MtBot and start the gateway.
...
7. Slash Commands → create `/mtbot` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
MtBot can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, MtBot falls back to the user token.
...
```json
{
  "display_information": {
    "name": "MtBot",
    "description": "Slack connector for MtBot"
  },
  "features": {
    "bot_user": {
      "display_name": "MtBot",
    ...
    "slash_commands": [
      {
        "command": "/mtbot",
        "description": "Send a message to MtBot",
        "should_escape": false
      }
    ]
  },
...
By default, MtBot replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, MtBot infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `mtbot pairing approve slack <code>`.
...
- On startup, MtBot resolves channel/user names in allowlists to IDs (when tokens allow)

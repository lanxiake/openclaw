...
- During heartbeat runs, MtBot treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `mtbot sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, MtBot skips the heartbeat run entirely (no model call).
...
```bash
mtbot system event --text "Check for urgent follow-ups" --mode now
```

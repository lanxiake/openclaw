...
ZIP=${1:?"Usage: $0 MtBot-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/mtbot/mtbot/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^MtBot-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/mtbot/mtbot/releases/download/v${VERSION}/"}

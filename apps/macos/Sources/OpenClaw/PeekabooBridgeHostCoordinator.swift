...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "com.yunkun.mtbot", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: MtBotPeekabooBridgeServices?
    private static var mtbotSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("MtBot", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = MtBotPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.mtbotSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.mtbotSocketPath, privacy: .public)")
    }
...
@MainActor
private final class MtBotPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "com.yunkun.mtbot.peekaboo")

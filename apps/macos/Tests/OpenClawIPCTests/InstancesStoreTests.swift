...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: MtBotProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: MtBotProtocol.AnyCodable] = [
            "presence": .init([MtBotProtocol.AnyCodable(entry)]),
        ]
        let payload = MtBotProtocol.AnyCodable(payloadMap)

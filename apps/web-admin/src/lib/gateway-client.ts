/**
 * Gateway WebSocket 客户端 (OpenClaw 协议)
 *
...
function sendConnectRequest(connectResolve: () => void, connectReject: (error: Error) => void): void {
  ...
  const connectParams: ConnectParams = {
    ...
    client: {
      ...
      displayName: 'OpenClaw Web Admin',
      version: '1.0.0',
      platform: 'web',
      mode: 'ui',
    },
  ...
  return new Promise((resolve, reject) => {
    ...
    ws.onclose = (event) => {
      ...
      if (reconnectAttempts < MAX_RECONNECT_ATTEMPTS) {
        reconnectAttempts++
        console.log(`[gateway] ${RECONNECT_DELAY / 1000}秒后尝试重连 (${reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS})`)
        setTimeout(() => {

...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.mtbot.top/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + MtBot hooks")
    ...
    .option("--hook-url <url>", "MtBot hook URL")
    .option("--hook-token <token>", "MtBot hook token")

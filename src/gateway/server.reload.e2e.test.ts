...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.MTBOT_SKIP_CHANNELS;
    prevSkipGmail = process.env.MTBOT_SKIP_GMAIL_WATCHER;
    process.env.MTBOT_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.MTBOT_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.MTBOT_SKIP_GMAIL_WATCHER;
    } else {
      process.env.MTBOT_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });

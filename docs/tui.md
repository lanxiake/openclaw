...
```bash
mtbot tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `mtbot tui --deliver`
...
- Check the Gateway logs: `mtbot logs --follow`.
- Confirm the agent can run: `mtbot status` and `mtbot models status`.
...
- No agents in picker: check `mtbot agents list` and your routing config.

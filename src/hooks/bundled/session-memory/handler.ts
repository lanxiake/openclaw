...
const saveSessionToMemory: HookHandler = async (event) => {
  ...
  try {
    logger.info("hook triggered for /new command");

    const context = event.context || {};
    const cfg = context.cfg as MtBotConfig | undefined;
    const agentId = resolveAgentIdFromSessionKey(event.sessionKey);
    const workspaceDir = cfg
      ? resolveAgentWorkspaceDir(cfg, agentId)
      : path.join(os.homedir(), ".mtbot", "workspace");
    ...
    if (sessionFile) {
      ...
      logger.debug("session content loaded", { length: sessionContent?.length || 0 });
      ...
      if (sessionContent && cfg) {
        ...
        const mtbotRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../..");
        const slugGenPath = path.join(mtbotRoot, "llm-slug-generator.js");
    ...
    logger.debug("memory file path", { filename, path: memoryFilePath });
    ...
    logger.info("memory file written successfully");

    // Log completion (but don't send user-visible confirmation - it's internal housekeeping)
    const relPath = memoryFilePath.replace(os.homedir(), "~");
    logger.info("session context saved", { path: relPath });
  } catch (err) {

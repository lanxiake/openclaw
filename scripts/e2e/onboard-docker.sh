...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/mtbot-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/mtbot-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/mtbot-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/mtbot"
    config_path="$HOME/.mtbot/mtbot.json"
    sessions_dir="$HOME/.mtbot/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.mtbot/mtbot.json" <<'"'"'JSON'"'"'
{

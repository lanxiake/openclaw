...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "mtbot-models-" });
}

const MODELS_CONFIG: MtBotConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureMtBotModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureMtBotModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureMtBotModelsJson } = await import("./models-config.js");
      const { resolveMtBotAgentDir } = await import("./agent-paths.js");

      await ensureMtBotModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveMtBotAgentDir(), "models.json");

...
vi.mock("./chrome.js", () => ({
  resolveMtBotUserDataDir: vi.fn(() => "/tmp/mtbot-test/mtbot/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "mtbot-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveMtBotUserDataDir).mockReturnValue(userDataDir);

...
Tlon is a decentralized messenger built on Urbit. MtBot connects to your Urbit ship and can
...
```bash
mtbot plugins install @mtbot/tlon
```

Local checkout (when running from a git repo):

```bash
mtbot plugins install ./extensions/tlon
```
...
Use these with `mtbot message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, MtBot replies in-thread.

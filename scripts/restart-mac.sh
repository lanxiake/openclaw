#!/usr/bin/env bash
# Reset MtBot like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${MTBOT_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="MtBot.app/Contents/MacOS/MtBot"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/MtBot"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/MtBot"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/MtBot"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/com.yunkun.mtbot.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/mtbot-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${MTBOT_RESTART_LOG:-/tmp/mtbot-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${MTBOT_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.mtbot/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  MTBOT_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node mtbot.mjs daemon install --force --runtime node"
      log "  node mtbot.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.mtbot/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/com.yunkun.mtbot.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing MtBot instances"
kill_all_mtbot
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product MtBot"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/MtBot.app" ]]; then
    APP_BUNDLE="/Applications/MtBot.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/MtBot.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/MtBot.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/MtBot.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set MTBOT_APP_BUNDLE to your installed MtBot.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node mtbot.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node mtbot.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".mtbot", "mtbot.json"), "utf8");

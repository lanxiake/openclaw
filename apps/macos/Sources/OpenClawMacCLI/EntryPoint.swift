...
struct MtBotMacCLI {
    static func main() async {
        ...
        default:
            fputs("mtbot-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      mtbot-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      mtbot-mac discover [--timeout <ms>] [--json] [--include-local]
      mtbot-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      mtbot-mac connect
      mtbot-mac connect --url ws://127.0.0.1:18789 --json
      mtbot-mac discover --timeout 3000 --json
      mtbot-mac wizard --mode local
    """)
}

...
describe("onboard (non-interactive): Vercel AI Gateway", () => {
  it("stores the API key and configures the default model", async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "mtbot-onboard-gateway-"));
    process.env.HOME = tempHome;
    process.env.MTBOT_STATE_DIR = tempHome;
    process.env.MTBOT_CONFIG_PATH = path.join(tempHome, "mtbot.json");
    vi.resetModules();

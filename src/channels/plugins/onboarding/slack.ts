...
function setSlackDmPolicy(cfg: MtBotConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "MtBot";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for MtBot`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: MtBotConfig, allowFrom: string[]): MtBotConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,

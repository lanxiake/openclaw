...
async function testACSub001(page) {
  ...
  try {
    await page.goto(`${CONFIG.baseUrl}/subscriptions`, { waitUntil: 'networkidle', timeout: CONFIG.timeout });
    ...
    // 检查统计卡片（总订阅数、活跃订阅等）
    const hasStats = pageContent.includes('订阅') && (
      pageContent.includes('总') || pageContent.includes('活跃') || pageContent.includes('试用') || pageContent.includes('收入')
    );

    if (hasTitle && hasCards > 0) {
      recordResult(id, name, 'PASS', `订阅管理页渲染正确：标题✓ 卡片${hasCards}个 搜索框${searchInput > 0 ? '✓' : '✗'} 统计${hasStats ? '✓' : '✗'}`);
    } else {
...
async function testACSub002(page) {
  ...
  try {
    ...
    // 检查是否有计划内容
    const hasPlanInfo = pageContent.includes('免费') || pageContent.includes('基础') ||
                        pageContent.includes('专业') || pageContent.includes('新建计划') ||
                        pageContent.includes('Plan') || pageContent.includes('price');

    if (hasTitle || hasPlanInfo) {
      recordResult(id, name, 'PASS', `计划页面渲染正确：标题${hasTitle ? '✓' : '✗'} 卡片${hasCards}个 计划信息${hasPlanInfo ? '✓' : '✗'}`);
    } else {
...
async function testACSub003(page) {
  ...
  try {
    await page.goto(`${CONFIG.baseUrl}/subscriptions/orders`, { waitUntil: 'networkidle', timeout: CONFIG.timeout });
    await page.waitForTimeout(2000);
    await takeScreenshot(page, `${id}-01-orders-page`);

    const pageContent = await page.textContent('body');
    const hasTitle = pageContent.includes('订单') || pageContent.includes('Orders');
    const hasContent = pageContent.includes('订单') || pageContent.includes('暂无') || pageContent.includes('没有');
...
async function testACGW001(page) {
  ...
  try {
    ...
    const pageContent = await page.textContent('body');
    const hasDashboardData = pageContent.includes('仪表盘') || pageContent.includes('Dashboard') ||
                             pageContent.includes('用户') || pageContent.includes('概览');

    if (hasDashboardData && gatewayStatus.authStorage === 'exists') {
      recordResult(id, name, 'PASS', `Gateway 连接正常, 仪表盘数据已加载, localStorage keys: ${gatewayStatus.localStorage.length}`);
    } else {
      recordResult(id, name, 'FAIL', `仪表盘数据:${hasDashboardData} authStorage:${gatewayStatus.authStorage}`);
    }
  } catch (err) {
    await takeScreenshot(page, `${id}-error`);
    recordResult(id, name, 'FAIL', err.message);
...
async function testACGW002(page) {
  ...
  try {
    ...
    if (hasUserSection && !hasError) {
      recordResult(id, name, 'PASS', `RPC 调用成功，页面正常显示（${hasData ? '空数据' : '有数据'}）`);
    } else {
...
async function testACGW004(page) {
  ...
  try {
    ...
    const hasError = pageContent.includes('连接失败') || pageContent.includes('Gateway 错误');
    const hasStats = pageContent.includes('用户') || pageContent.includes('订阅') || pageContent.includes('活跃');

---
summary: "First-run onboarding flow for MtBot (macOS app)"
read_when:
...
- Writes credentials to `~/.mtbot/credentials/oauth.json`
...
The app can install the global `mtbot` CLI via npm/pnpm so terminal
...
On the first agent run, MtBot bootstraps a workspace (default `~/.mtbot/workspace`):
...
```bash
mtbot webhooks gmail setup --account you@gmail.com
```

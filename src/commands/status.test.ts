...
beforeAll(() => {
  previousProfile = process.env.MTBOT_PROFILE;
  process.env.MTBOT_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.MTBOT_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/mtbot-root.js", () => ({
  resolveMtBotPackageRoot: vi.fn().mockResolvedValue("/tmp/mtbot"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/mtbot/pnpm-lock.yaml",
      markerPath: "/tmp/mtbot/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("MtBot status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("mtbot status --all") ||
          l.includes("mtbot --profile isolated status --all") ||
          l.includes("mtbot status --all") ||
          l.includes("mtbot --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.MTBOT_GATEWAY_TOKEN = "abcd1234";
    try {

...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/mtbot/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "MtBot Re-Auth" -c "After getting key, run: ssh $SERVER '~/mtbot/scripts/mobile-reauth.sh'" --id mtbot-auth
        ;;
    *)

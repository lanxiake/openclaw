...
read_when:
  - Setting up Twitch chat integration for MtBot
---

# Twitch (plugin)

Twitch chat support via IRC connection. MtBot connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
mtbot plugins install @mtbot/twitch
```

Local checkout (when running from a git repo):

```bash
mtbot plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `MTBOT_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "mtbot", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use MTBOT_TWITCH_ACCESS_TOKEN env var)
...
```bash
MTBOT_TWITCH_ACCESS_TOKEN=oauth:abc123...
```

...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.mtbotCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.mtbotCanvasA2UIAction &&
        typeof window.mtbotCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.mtbotSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("mtbot:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android MtBot node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.mtbotSendUserAction === "function"
        ? window.mtbotSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.MTBOT_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.MTBOT_SKIP_CANVAS_HOST)) {
    return true;
...
function resolveDefaultCanvasRoot(): string {
  const candidates = [path.join(os.homedir(), ".mtbot", "canvas")];
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>MtBot Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;

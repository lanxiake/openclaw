...
# Checks both Claude Code and MtBot auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
MTBOT_AUTH="$HOME/.mtbot/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    mtbot models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    mtbot_status=$(check_mtbot_auth 2>/dev/null || true)

    claude_expires=0
    mtbot_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        mtbot_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        mtbot_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$MTBOT_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        --arg bs "$mtbot_status" \
        --arg be "$mtbot_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            mtbot: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$mtbot_status" == EXPIRED* ]] || [[ "$mtbot_status" == MISSING* ]]; then
        echo "MTBOT_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$mtbot_status" == EXPIRING* ]]; then
        echo "MTBOT_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "MtBot Auth (~/.mtbot/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$MTBOT_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$MTBOT_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'mtbot doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active mtbot >/dev/null 2>&1; then
    echo -e "MtBot service: ${GREEN}running${NC}"
else
    echo -e "MtBot service: ${RED}NOT running${NC}"
fi

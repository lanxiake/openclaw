...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["MTBOT_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/mtbot")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("mtbot-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("mtbot-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/mtbot/ (rolling or stdout), or nil if none exist.

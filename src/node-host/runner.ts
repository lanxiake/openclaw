...
const execHostEnforced = process.env.MTBOT_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.MTBOT_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureMtBotCliOnPath({ pathEnv: process.env.PATH ?? "" });

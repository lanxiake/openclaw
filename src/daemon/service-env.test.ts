...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.MTBOT_GATEWAY_PORT).toBe("18789");
    expect(env.MTBOT_GATEWAY_TOKEN).toBe("secret");
    expect(env.MTBOT_SERVICE_MARKER).toBe("mtbot");
    expect(env.MTBOT_SERVICE_KIND).toBe("gateway");
    expect(typeof env.MTBOT_SERVICE_VERSION).toBe("string");
    expect(env.MTBOT_SYSTEMD_UNIT).toBe("mtbot-gateway.service");
    if (process.platform === "darwin") {
      expect(env.MTBOT_LAUNCHD_LABEL).toBe("com.yunkun.mtbot.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", MTBOT_PROFILE: "work" },
      port: 18789,
    });
    expect(env.MTBOT_SYSTEMD_UNIT).toBe("mtbot-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.MTBOT_LAUNCHD_LABEL).toBe("com.yunkun.mtbot.work");
    }
  });
});

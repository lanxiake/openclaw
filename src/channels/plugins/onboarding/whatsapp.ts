...
function setWhatsAppDmPolicy(cfg: MtBotConfig, dmPolicy: DmPolicy): MtBotConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: MtBotConfig, allowFrom?: string[]): MtBotConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: MtBotConfig, selfChatMode: boolean): MtBotConfig {
...
async function detectWhatsAppLinked(cfg: MtBotConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so MtBot can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for MtBot" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("mtbot channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }

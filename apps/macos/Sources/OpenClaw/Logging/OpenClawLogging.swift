...
enum MtBotLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = MtBotOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = MtBotFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        MtBotLogging.bootstrapIfNeeded()
        let label = MtBotLogging.makeLabel(subsystem: subsystem, category: category)
...
struct MtBotOSLogHandler: LogHandler {
...
struct MtBotFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = MtBotLogging.parseLabel(self.label)

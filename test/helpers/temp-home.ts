...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("MTBOT_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.MTBOT_STATE_DIR = path.join(base, ".mtbot");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "mtbot-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".mtbot", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {

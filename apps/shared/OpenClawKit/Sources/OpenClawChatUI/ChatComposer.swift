...
@MainActor
struct MtBotChatComposer: View {
    @Bindable var viewModel: MtBotChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(MtBotChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(MtBotChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \MtBotPendingAttachment.id)
                { (att: MtBotPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            MtBotPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(MtBotChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(MtBotChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(MtBotChatTheme.subtleCard)
        .clipShape(Capsule())
    }

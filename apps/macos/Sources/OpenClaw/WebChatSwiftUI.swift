...
private let webChatSwiftLogger = Logger(subsystem: "com.yunkun.mtbot", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: MtBotChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> MtBotChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(MtBotChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> MtBotChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                MtBotGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<MtBotChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any MtBotChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = MtBotChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: MtBotChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<MtBotChatView>) -> NSViewController
    {

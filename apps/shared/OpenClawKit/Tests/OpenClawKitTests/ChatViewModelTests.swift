...
private final class TestChatTransport: @unchecked Sendable, MtBotChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [MtBotChatHistoryPayload]
    private let sessionsResponses: [MtBotChatSessionsListResponse]

    private let stream: AsyncStream<MtBotChatTransportEvent>
    private let continuation: AsyncStream<MtBotChatTransportEvent>.Continuation

    init(
        historyResponses: [MtBotChatHistoryPayload],
        sessionsResponses: [MtBotChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<MtBotChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> MtBotChatHistoryPayload {
        ...
        return self.historyResponses.last ?? MtBotChatHistoryPayload(
        ...
        attachments _: [MtBotChatAttachmentPayload]) async throws -> MtBotChatSendResponse
    {
        ...
        return MtBotChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> MtBotChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? MtBotChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: MtBotChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = MtBotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = MtBotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { MtBotChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = MtBotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = MtBotChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = MtBotChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { MtBotChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }

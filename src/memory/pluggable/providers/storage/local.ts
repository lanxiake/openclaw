...
export class LocalObjectStorageProvider implements IObjectStorageProvider {
   ...
   * 保存元数据
   */
  private async saveMetadata(bucket: string, key: string, metadata: Record<string, string>): Promise<void> {
  ...
  async list(bucket: string, prefix?: string, options?: StorageListOptions): Promise<ListResult> {
    ...
    return {
      ...
      continuationToken: objects.length > limit ? truncatedObjects[truncatedObjects.length - 1].key : undefined,
    }
  }
...
// 自动注册提供者
registerProvider('storage', 'local', LocalObjectStorageProvider as unknown as new (options: Record<string, unknown>) => LocalObjectStorageProvider)

...
private let chatUILogger = Logger(subsystem: "com.yunkun.mtbot", category: "MtBotChatUI")

@MainActor
@Observable
public final class MtBotChatViewModel {
    public private(set) var messages: [MtBotChatMessage] = []
    ...
    public var attachments: [MtBotPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [MtBotChatPendingToolCall] = []
    public private(set) var sessions: [MtBotChatSessionEntry] = []
    private let transport: any MtBotChatTransport
    ...
    private var pendingToolCallsById: [String: MtBotChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any MtBotChatTransport) {
    ...
    public var sessionChoices: [MtBotChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [MtBotChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [MtBotChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: MtBotPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [MtBotChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: MtBotChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [MtBotChatMessage]) -> [MtBotChatMessage] {
        var result: [MtBotChatMessage] = []
    ...
    private static func dedupeKey(for message: MtBotChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [MtBotChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> MtBotChatAttachmentPayload in
            MtBotChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> MtBotChatSessionEntry {
        MtBotChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: MtBotChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: MtBotChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: MtBotAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = MtBotChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> MtBotPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)

...
- Replaces the legacy `gateway` command. CLI entry point: `mtbot gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)

```bash
mtbot gateway --port 18789
...
- Config hot reload watches `~/.mtbot/mtbot.json` (or `MTBOT_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__mtbot__/canvas/` from `~/.mtbot/workspace/canvas`. Disable with `canvasHost.enabled=false` or `MTBOT_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `MTBOT_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `MTBOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.mtbot.*` may still exist)
- Linux: `mtbot-gateway-<profile>.service`
- Windows: `MtBot Gateway (<profile>)`

Install metadata is embedded in the service config:

- `MTBOT_SERVICE_MARKER=mtbot`
- `MTBOT_SERVICE_KIND=gateway`
- `MTBOT_SERVICE_VERSION=<version>`
...
```bash
mtbot --dev setup
mtbot --dev gateway --allow-unconfigured
...
- `MTBOT_STATE_DIR=~/.mtbot-dev`
- `MTBOT_CONFIG_PATH=~/.mtbot-dev/mtbot.json`
- `MTBOT_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.mtbot/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `MTBOT_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `MTBOT_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `MTBOT_CONFIG_PATH`
- unique `MTBOT_STATE_DIR`
...
```bash
mtbot --profile main gateway install
mtbot --profile rescue gateway install
```

Example:

```bash
MTBOT_CONFIG_PATH=~/.mtbot/a.json MTBOT_STATE_DIR=~/.mtbot-a mtbot gateway --port 19001
MTBOT_CONFIG_PATH=~/.mtbot/b.json MTBOT_STATE_DIR=~/.mtbot-b mtbot gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `mtbot health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `mtbot`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `mtbot gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.mtbot.*` is cleaned up).
  - `mtbot doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are MtBot profile services.
  ...
  - Cleanup: `mtbot gateway uninstall` (current service) and `mtbot doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `mtbot gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- MtBot.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.mtbot.*` labels still unload cleanly).
- To stop it cleanly, use `mtbot gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `mtbot gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `mtbot gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

MtBot installs a **systemd user service** by default on Linux/WSL2. We
...
`mtbot gateway install` writes the user unit. `mtbot doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/mtbot-gateway[-<profile>].service`:

```
[Unit]
Description=MtBot Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/mtbot gateway --port 18789
Restart=always
RestartSec=5
Environment=MTBOT_GATEWAY_TOKEN=
...
```
systemctl --user enable --now mtbot-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/mtbot-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now mtbot-gateway[-<profile>].service
```
...
- `mtbot gateway health|status` — request health/status over the Gateway WS.
- `mtbot message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `mtbot agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `mtbot gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `mtbot gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `mtbot gateway` and the legacy TCP control port.
